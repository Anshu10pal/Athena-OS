...
| — | **Cold ingest — next session's first task.** Largest unknown about real behaviour; gates the MCP design question (a first call cannot wait on a two-hour analysis). Needs a quiet machine, back-to-back runs. Predict with the three-part decomposition in contract §17.0b — **not** by dividing 14.4 s by `files_parsed: 5`, which is the category error that produced that section's third clause. Output is honestly "cold ingest of *superset* takes N", one Python monolith — a TypeScript monorepo with more, smaller files and more edges would stress parse and resolve differently | **next** |
