...
| — | **`evict_lru_if_needed` orphans every child row it should delete.** `registry.py`'s LRU cache eviction calls `db.delete(r)` on the `Repo` row alone. `Repo` declares no ORM relationships to `code_files`/`code_symbols`/`code_imports`/`code_file_ranks`/`code_subsystems`/`code_health_snapshots`/`code_file_health`/`repo_jobs`, and every foreign key is `ON DELETE NO ACTION`, so **nothing cascades** — an eviction leaves the whole analysis behind, pointing at a repo id that no longer exists, invisible to every query that starts from `repos`. Pre-existing and now trivially fixable: `deletion.delete_repo` does exactly this correctly, including the `code_files ↔ code_subsystems` cycle. Eviction should call it instead of hand-rolling a one-line delete. Found while running down an unexplained repo disappearance, which it turned out **not** to explain — eviction targets `source_kind == "clone"` only, and would have left orphans there were none of | **open** |
| — | **Every server log captured this session was EMPTY, including the ones meant to be the record for the open crash.** Python block-buffers stdout when it is redirected to a file rather than a terminal, and a dev server is ended by killing it, so the buffer is never flushed. Measured: plain → nothing while alive and nothing after kill; `-u` or `sys.stdout.reconfigure(line_buffering=True)` → the line is present in both. So the commit that said "access logging is deliberately on and must not be suppressed for quiet" was **true about configuration and false about outcome** — the same shape as the uvicorn formatter no-op two entries down, except this one had already been relied on. Fixed in `run.py`; any harness launching `python -m uvicorn` with redirected output must pass `-u` | **closed** |
| — | **Repo 5 disappeared and I cannot explain it.** Present at session start with 43 rows across five tables, present in the listing after the throwaway-repo cleanup, absent now — with **no orphaned rows**, which means a complete removal across all eight tables. The only code that does that is `deletion.delete_repo`, and no run of it targeted repo 5: the cleanup filtered `name like 'athena-owned-%'` and repo 5 was named `repo`. LRU eviction is ruled out twice (it targets clones only, and it would have left orphans). No server log survived to check, for the reason in the row above. Recorded as unexplained rather than given a plausible story; a recurrence is now diagnosable because the logging is fixed | **open, unexplained** |
