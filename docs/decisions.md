...
**`:682` is finished, so the next work is a choice between three deferred
items. None is started; the tradeoffs, not a recommendation:**
...
**Also outstanding, small:** run a full backend suite. The last one (1,109
passed) predates checkpoint 3's 4 filter tests, which were verified in isolation
only.
