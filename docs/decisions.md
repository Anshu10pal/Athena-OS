...
| — | **CHECKPOINT 5 — THE HOOK IS BUILT AND STRICT WAS CHOSEN, AND ITS EXTENSION LAYER HAS NEVER FIRED. Two sessions, two non-fires, zero harness invocations.** `backend/scripts/athena_read_hook.py` exists with **strict** selected, five preconditions, fail-open on every path, and **15 tests** in `test_read_hook.py` (suite 1,181 -> **1,196**, reconciling exactly). **The record was behind the code:** README and `phase6-graph-as-context.md` §9 both still framed nudge-vs-strict as "decide that before building" while the strict hook was committed and tested. **THE EXTENSION-LAYER CLAIM THAT WAS NEVER MADE, AND THE ONE THAT WAS.** A live-observation session (2026-09-01, `docs/live-observations/session-01.md`) tripped a "hook did not fire" stop condition. Diagnosis from the transcripts, not from inside the failure: **the prior session's apparent confirmation was a Bash `tool_result` (its own stdout from a synthetic invocation with `ATHENA_HOOK_STATE_DIR` redirected to a scratchpad probe, `session_id="probe-cold-check"`), and the ONE real `Read` it issued three entries later returned the file's contents uninterrupted** (`da076d61…jsonl` lines 89 and 93/94). **§17.16 applied to a DECISION rather than a number: the instrument that produced the deny was never reported alongside it, and a script's stdout was read as an extension-layer verdict.** The probe marker survives on disk as the receipt. **THE MECHANISM, three candidates separated.** **(1) Settings location — CONFIRMED, and sufficient on its own.** No `.claude/` exists at or above the session cwd `/home/hack-t36/Athena`; the only one is two levels down at `athena-os/.claude/`. **Corroborated by contrast in the same session: `.mcp.json` sits AT the workspace root and loaded fine** (five `neighborhood` calls), so cwd is demonstrably where this client reads project config. **(2) Schema — LATENT SECOND FAULT, failure mode confirmed empirically.** The entry uses `{"type":"command","command":<python>,"args":[<script>]}`; Claude Code's PreToolUse shape is a single `command` string with no `args`. Piping a payload to the bare interpreter exits **0 with no output** — a JSON object literal is a valid Python expression statement — so an ignored `args` is **silently indistinguishable from the hook not being registered at all**. Cannot be observed as active until (1) is fixed, and will bite immediately after. **(3) `cat` routing — REAL, NOT CAUSAL.** This session ran under an auto-mode instruction to read files via Bash `cat`/`head`/`sed`, and a `Read` matcher cannot see those (1 `Read` against 4+ `cat` here). **But it is disproven as the mechanism by a counterexample in EACH session:** a genuine `Read` tool_use occurred both times and was not intercepted. It remains a standing reliability limit — the hook's reach depends on a tool-choice heuristic it does not control — and that is a design conversation, not a bug. **WHY THIS IS THE 4b LESSON UNLEARNED ONE CHECKPOINT LATER.** 4b's closure row records, in its own words, that `.mcp.json` had to be **recreated at the workspace root, outside the repo**, because the client's cwd is not `backend/`. Checkpoint 5 then placed its config two levels BELOW that same workspace root. **Same class of defect, same session-day, one checkpoint apart, with the fix already written down.** **NOT CORRECTED HERE, because it is TRUE: 4b's "closed at both layers" is about the MCP server, not the hook** — an extension-layer `neighborhood` call, independently reproduced **five times** in the observation session. That closure stands. **No fix applied in this pass; diagnosis only, per instruction** *(**SUPERSEDED IN PART 2026-09-01** — the extension layer has since FIRED and checkpoint 5 is CLOSED at that layer; see the confirmation row below. Of the three candidate mechanisms recorded here, **(1) settings location was RIGHT IN KIND BUT NOT IN DETAIL**: a `.claude/settings.json` at the session cwd `/home/hack-t36/Athena` is indeed what the client reads — one now exists there — but the operative fault was that its interpreter and script paths were RELATIVE, and the cwd sits one level ABOVE the repo root `…/Athena/athena-os`, so they resolved to missing files and the hook could not be spawned. **(2) The schema concern did NOT materialise:** the working entry uses exactly the `{"type":"command","command":…,"args":[…]}` shape this row predicted would be silently ignored, and it fires — so `args` IS honoured by this client and the "latent second fault" is withdrawn. **(3) `cat` routing remains real and non-causal, as recorded.** The lasting lesson is unchanged and is strengthened: a hook whose command cannot be resolved fails SILENTLY, which is why a wrong mechanism survived three observations.)* | **checkpoint 5: BUILT AND TESTED AT THE SCRIPT LAYER; EXTENSION LAYER UNCONFIRMED — never once invoked by the harness. Phase 6 is NOT end-to-end** *(status as of this row; **SUPERSEDED 2026-09-01 — extension layer CONFIRMED, checkpoint 5 CLOSED at both layers**)* |
| — | **CHECKPOINT 5 — EXTENSION LAYER CONFIRMED, AND PHASE 6 IS DONE END TO END.** This is the confirmation row the entry above promises and which did not exist until now — the supersession note pointed at a row nobody wrote, so for two days the record asserted a closure whose evidence was not in it. **Confirmed 2026-09-03**, session `da076d61-e756-4ee6-a3d9-829343a342cc`, on the Linux VM. **THE DENY CAME BACK AS A `Read` TOOL ERROR FROM THE HARNESS, WHICH IS THE WHOLE POINT** — not a Bash `tool_result` carrying the script's own stdout, which is precisely the instrument confusion the row above had to retract. Receipt on disk, written by the hook itself and not by the session reporting it: `~/.cache/athena-read-hook/session-da076d61-e756-4ee6-a3d9-829343a342cc.fired` (17:17 UTC). The state directory was the REAL one (`/home/hack-t36/.cache/athena-read-hook/`), not a scratchpad probe, and `ATHENA_HOOK_STATE_DIR` was unset. **THE FULL LOOP CLOSED, WHICH IS THE ACTUAL DELIVERABLE:** the hook denied a read of `backend/app/services/codebase/health_scoring.py`, naming `repo="Anshu10pal/Athena-OS"` (identity-resolved, not path-matched) and the correct repo-relative path; the redirect was then FOLLOWED, and `mcp__athena-graph__neighborhood` answered from the real graph — **rank 31, fan_in 7, fan_out 0, 7 importers all enriched and all same-subsystem, 3 unresolved (`dataclasses` ×2, `typing`), snapshot `5155f6ceb9f6` / 2026-08-20, budget 9,000 `applied: false`.** Deny → query → usable answer, through the harness, with no step simulated. **TWO CORRECTIONS TO THE SUPERSEDED DIAGNOSIS ABOVE, per §17.16.** (1) The relative-paths account of the mechanism — repeated in the README as "all four verified missing" — **is not supported by what is on disk, and the operative fault was LOCATION.** `/home/hack-t36/Athena/.claude/settings.json` is **byte-identical** to the committed `athena-os/.claude/settings.json` (`diff` clean) and shares its mtime to the nanosecond (`13:06:05.628374302`), while the `.claude/` directory holding it is stamped `15:44` — i.e. it is a mtime-preserving COPY of the committed file, placed at the cwd, and **it carries fully ABSOLUTE interpreter and script paths.** There is no on-disk trace of a relative-path version at the cwd, and the copy fired 8 minutes after it appeared. The file did not need rewriting; it needed to be somewhere the client looks. **Left as a marked correction rather than an overwrite (§17.16): a plausible mechanism was recorded as "now proven" on the strength of the fire that followed it, when the fire is equally explained by the copy — two changes were made and only one was credited.** (2) The `args` withdrawal is now positively confirmed rather than merely withdrawn — the firing entry uses `{"type":"command","command":…,"args":[…]}` and this client honours it. **WHAT IS PROVEN AND WHAT IS NOT, because they are different claims (§15.1).** Proven: the hook fires at the extension layer, denies, and its redirect is answerable. **NOT proven by this session: that it fires on the FIRST raw source read of a session.** The first `Read` here (`ranking.py`, equally qualifying) went through un-intercepted, and `.claude/settings.json` at the session cwd was observed APPEARING between two Bash calls in this same session — so precondition 2 was never exercised against a loaded hook, and the first-read-of-session behaviour remains attested only at the script layer by its 15 tests. Recorded rather than smoothed over: the session that closed this checkpoint also contains one un-intercepted qualifying read, and a closure row that hid that would be the §17.30 failure again | **CLOSED 2026-09-03 — both layers. PHASE 6 IS DONE END TO END: graph exported, read boundary defined, neighbourhood queryable, MCP server exposed, hook enforcing. Residual: first-read-of-session ordering unproven at the extension layer** |
| — | **FIRST LIVE-USE SESSION — two findings a benchmark could not have produced. Full detail in `docs/phase6-graph-as-context.md` §9.1; log in `docs/live-observations/session-01.md`.** **(A) THE GRAPH ANSWERED A TASK WITH NO CHECKOUT PRESENT, and checkpoint 3 never measured this.** A real Superset investigation (trace connection handling, name the config flag, assess blast radius) was answered with **five `neighborhood` calls + six SQL queries and ZERO source reads — because zero were possible: there is no Superset anywhere on this disk** (searched `/` to depth 8). Checkpoint 3's 219.7x measures *cheaper than reading files you have*; its denominator presumes local source. Here the alternative was not reading 560,768 tokens, it was **cloning 6,584 files first**. **That is a different and stronger claim — the graph as a queryable SUBSTITUTE for a repo you do not have, for structural questions** (onboarding, blast radius, "what would this break"). **Recorded as EVIDENCE, NOT A RESULT: one session, one repo, one task**, and its weakest sub-answer was weak for exactly the reason in (B). What it justifies is a new measurement — across repos and question classes, including where it breaks — not the claim in marketing voice, and **not a re-run of checkpoint 3's benchmark**. **(B) `neighborhood` ANSWERS "what depends on this FILE" WHEN THE REAL QUESTION IS OFTEN "what depends on this SYMBOL", and the data is ALREADY IN `athena.db`.** `superset/utils/core.py` reports `fan_in: 346`, which reads as "enormous blast radius, tread carefully". Its **connection-handling production fan-in is ONE** (`pessimistic_connection_handling` <- `superset/initialization/__init__.py:84`), plus two single-caller SSL helpers (`parse_ssl_cert` <- `databases/schemas.py:61`, `create_ssl_cert_file` <- `db_engine_specs/trino.py:53`). **File-level fan-in answered a question nobody asked and pointed the opposite way from the truth.** The real answer came from **`code_imports.imported_names`** and **`code_symbols`** — both persisted, neither exposed by the API. Split: **5 tool calls for orientation, 6 raw SQL queries for the answer.** Two sub-gaps: symbol-level neighbourhood is **additive and cheap** on existing data (same shape as the deferred `path`/`explain` primitives); and **module-level constants are not captured as symbols at all** (`superset/config.py` -> 15 symbols for a file reaching line 3,323), so **config-flag questions are unanswerable on any Flask-`current_app.config` codebase** — flag consumption leaves no import edge either (`config.py` total fan-in **10**). That second one may be a legitimate scope boundary rather than a gap; **name it either way** so it is not rediscovered. **Neither is a bug** — the tool returns what checkpoint 2 built. **NOT ACTED ON: exposing symbols is a design decision, not a correction** | **recorded; (A) needs its own measurement, (B) deferred to the next Phase 6 work** |
| — | **DECISION — THE HOOK'S SCOPE IS THE `Read` TOOL. Bash-mediated source reads (`cat`/`head`/`sed`/`grep`) are OUT OF SCOPE, deliberately.** Decided 2026-09-01; reasoning in `docs/phase6-graph-as-context.md` §9.2. Four options weighed: PostToolUse-on-Bash (coverage without redirect), a Bash matcher analysing shell commands (redirect at high cost), Read-only with the boundary documented, Read-only with it unstated. **Chosen: Read-only, boundary DOCUMENTED.** **(1) Audience** — interactive Claude Code use opens source with `Read`. **(2) A PRECISE PARTIAL BOUNDARY BEATS A FUZZY FULLER ONE** — "`Read` in scope, Bash not" is checkable; shell analysis yields "Bash mostly covered, depending on command shape", which is softer and easier to drift on. **This is §17.33/§17.36's logic applied to a COVERAGE claim rather than a number.** **(3) Cost** — `cat`/`head`/`sed -n`/pipes/redirects/`find | xargs cat`/quoted and globbed paths is checkpoint-sized, not a patch; build it from observed need. **(4) The problem is not current** — automatic redirect in autonomous Bash-first sessions matters only under widespread agentic use. **THE REVERSING SIGNAL, made countable rather than left to opinion:** type-**B** entries (*wanted the hook to fire, it did not*) accumulating in `docs/live-observations/` specifically on `cat`-routed reads where the miss cost something. **[STATUS, §17.16 — the SCOPE and the CONFIRMATION are separate; BOTH are now settled.] Scope: DECIDED. `Read`-tool confirmation: CONFIRMED 2026-09-01.** **The three earlier `Read` non-fires** — `ranking.py` in the earlier session (`da076d61…jsonl`, tool_use line 93, contents returned line 94), `graph_structure.py` in the observation session, and `ranking.py` re-tested in the diagnosis session — **are now explained, and the explanation previously recorded in this row was WRONG.** It was not a `/hooks` registration postdating session start. **It was path resolution: Claude Code is spawned from `/home/hack-t36/Athena`, one level ABOVE the repo root `/home/hack-t36/Athena/Athena/athena-os`,** so the relative interpreter/script paths then in `settings.json` resolved to missing files — `.venv/bin/python`, `venv/bin/python`, `backend/venv/bin/python` and `backend/scripts/athena_read_hook.py` all verified MISSING from that cwd this session, the absolute forms now in place verified EXISTS. **A hook whose command cannot be resolved FAILS SILENTLY — no error reaches the session and the `Read` simply returns contents — which is exactly why a wrong hypothesis survived three observations.** **THE CONFIRMING OBSERVATION:** cold session `0d024641-c814-4d5d-9d23-dfe72f513721`, one real `Read` of `backend/app/services/codebase/ranking.py`, **DENIED** by the harness with the checkpoint-5 strict-mode message, and marker `~/.cache/athena-read-hook/session-0d024641-c814-4d5d-9d23-dfe72f513721.fired` written containing the attempted path, **session id in the filename matching the observing session.** **WHY THIS IS A MECHANISM AND NOT A SECOND ANECDOTE:** the only prior marker was `session-DIAG-manual.fired`, a hand-triggered invocation under a synthetic id — it proved the hook script ran, not that the harness would ever spawn it; this fire needed no priming and was taken from the very cwd where the old relative paths do not resolve, so it is **independent of the earlier one-off's circumstances**, and that independence is the upgrade from single observation to established mechanism. **BEHAVIOURAL DETAIL WORTH KEEPING:** the session’s *first* `Read` used the repo-relative path as written, which does not exist from the spawn cwd; it returned a plain "File does not exist" error, did **not** fire the hook and did **not** consume the once-per-session fire — so a wrongly-rooted path neither spends the interception nor counts as a type-**B** non-fire, and any future non-fire report must confirm the path existed first. **Nothing built, nothing modified this session: observation plus docs only — hook code, `settings.json` and the neighborhood tool untouched.** **Checkpoint 5 is CLOSED at the extension layer** | **scope DECIDED; `Read` confirmation CONFIRMED 2026-09-01 (cold session `0d024641…` — denied + matching marker)** |
...
| — | **PHASE 8 — GRAPH AS CONTEXT, THE USER SURFACE. Two design decisions, pinned before any code.** **THE RENUMBER, DECIDED 2026-09-04 — the collision this row was parked against is RESOLVED.** This UI work is **Phase 8**. **Phase 7 KEEPS multi-language expansion** (Go/Rust/Java/C# tree-sitter extractors — the row directly above, untouched and still deferred). The ~10%-designed **drift detection moves to Phase 9 / unnumbered-until-designed**, so it is not holding a number it has not earned. **Phase 8 was previously recorded as "NOT STARTED AND NOT DEFINED — the number is a placeholder, not a planned phase"; that placeholder is now spent, deliberately and by a human decision rather than by a session inferring a scope from the numbering.** This row was originally parked adjacent to Phase 7 with the collision flagged rather than merged into it; that parking was confirmed correct and is folded in here. **DECISION 1 — PLACEMENT: a new Context tab in `RepoDetail`, NOT a top-level section.** Verbatim: *"a new Context tab in RepoDetail (not a top-level section — the reconnaissance confirmed there's no top-level atlas view to sit below, and a new top-level section would fight the repo-centric architecture). The prominence the feature needs comes from the tab's content being self-legible, not from nav placement."* **This carries a binding constraint onto checkpoints 3–4 (the visualisation work), recorded here so it cannot be lost between deciding it and building it: the Context view MUST read clearly to someone who has just navigated to it, with the token-savings number prominent.** The argument is that placement was traded away deliberately, so legibility is not a nice-to-have that can be dropped under time pressure — it is the compensating mechanism for the placement decision. **DECISION 2 — TOKEN MATH: ONE estimator, `_estimate_tokens`, on BOTH sides of the ratio.** Verbatim: *"use the existing `_estimate_tokens` heuristic (`neighborhood.py`, the ~3.6 chars/token method already used for the budget cap) for both sides of the savings ratio — the neighborhood's own token cost (already computed by the neighborhood function) and the 'reading the connected files' total (estimated from each connected file's `size_bytes` via the same heuristic). Do NOT introduce a separate line-count-based estimate — that would be a second instrument measuring the same thing differently (§17.28). One estimator, consistent with the tool's own budget accounting. The UI will state it's an estimate and name the method."* **Why this is a §17.28 guard and not a style preference:** the endpoint's displayed saving and the MCP tool's budget accounting would be two instruments measuring the same quantity, and if they disagree the disagreement is invisible to the user reading the number. **CORRECTION TO DECISION 2'S PREMISE, found during checkpoint 1 recon (§17.16 — marked, not silently fixed): the parenthetical "already computed by the neighborhood function" is FALSE on both counts.** `read_neighborhood` does not surface its own token cost (`_apply_budget` records only `{limit, applied}`, never the count), and `_describe` does not emit `size_bytes` at all — so neither side of the ratio is currently obtainable from the function's return value. The decision is sound; its stated starting point is not. See the checkpoint 1 blocker row | **DECISIONS PINNED 2026-09-03; PHASE NUMBER RESOLVED 2026-09-04 — this is PHASE 8, Phase 7 keeps multi-language, drift detection moves to Phase 9/unnumbered. Checkpoint 1 UNBLOCKED (route (a), denominator Option B) and NOT YET BUILT** |
| — | **PHASE 8 CHECKPOINT 1 — THE TWO RECON BLOCKERS, kept in full because the diagnosis is the record (§17.16). BOTH ARE NOW ANSWERED — see the decisions row directly below; nothing here is retracted, it is the reasoning the answers were chosen against.** **BLOCKER 1 — THE ROUTE CONVENTION DOES NOT EXIST.** The brief said to match an existing `?file=` query-param convention. **There is no such convention in `repos.py`.** A grep for `file_path` / `path: str` / `file: str` / `Query(` across all 27 repo-scoped routes returns only `segments` and `languages` (both `list[str]` filters on `/graph`). **The ONLY file-scoped route is `GET /{repo_id}/files/{file_id}/neighbors` (`repos.py:607`), which addresses a file by INTEGER `file_id`, never by path.** So there is nothing to match, and per the brief's own instruction the options are reported rather than one being picked: **(a)** `/{repo_id}/files/{file_id}/context` — matches the one existing precedent exactly, sidesteps URL-encoding entirely, and costs one `CodeFile` id→path lookup since `read_neighborhood` takes a path; **(b)** `/{repo_id}/context?file=<path>` — reads naturally and needs no id, but sets a NEW convention in this router and requires percent-encoding of `/`; **(c)** `/{repo_id}/files/by-path/{path:path}` — FastAPI path converter, no encoding of `/` needed but ambiguous against the existing `files/{file_id}` route and awkward for paths with `.` segments. **BLOCKER 2 — `read_neighborhood` EXPOSES NEITHER SIDE OF THE RATIO, and one of them cannot be fixed by surfacing alone.** (i) **Its own token cost is not in the result.** `_apply_budget` writes only `{"limit": n, "applied": bool}`; the count is computed and discarded. The MCP server's logged "estimate 5,062" is `_estimate_tokens(result)` recomputed at the call site. Cheap to fix — surface it in the result, same estimator, no behaviour change. (ii) **`size_bytes` is not emitted, and adding it does not make the denominator correct.** `_describe` (`neighborhood.py:150`) emits only `p`/`rank`/`cluster`/`fan_in`/`crosses`; `NodeT` DOES carry `size_bytes` (`graph_read.py:100`), so the data exists one layer down. **But `MAX_ENRICHED = 25`**: on `superset/utils/core.py`'s 258 importers, 25 entries get metadata and **233 are bare path strings in `additional_paths`** — and under budget truncation `files` empties entirely and ALL of them become bare paths. **So a "connected files total" summed over what the result exposes would cover 25 of ~280 files while presenting itself as the cost of reading the connected set — a denominator that silently excludes 90% of its population, which is §17.5c exactly, wearing a §17.25 face: the consumer cannot tell the number is partial.** **THE REAL DECISION THIS SURFACES, which is why it was not resolved unilaterally:** widening `_describe` to emit `size_bytes` for all connected files would inflate EVERY neighbourhood answer the MCP tool returns, changing the budget accounting and the measured Phase 6 saving figures (219.7x pooled) that were taken against the current payload shape. **A UI convenience would silently re-price the tool's own headline measurement.** The alternatives — a separate opt-in `include_sizes` flag, or a distinct service function that returns sizes without touching the MCP payload — are cheaper but are a design call, not a build detail | **RAISED 2026-09-03, both ANSWERED 2026-09-04: route = option (a); denominator = Option B. Reported rather than worked around, which is why the answers exist** |
| — | **PHASE 8 CHECKPOINT 1 — THE TWO ANSWERS, decided 2026-09-04. Both were reported as options rather than picked by the session that found them, and both are decided here by a human.** **ANSWER 1 — ROUTE SHAPE: option (a), `GET /{repo_id}/files/{file_id}/context`.** Chosen because it is the **only option with precedent** in this router — `GET /{repo_id}/files/{file_id}/neighbors` (`repos.py:607`) already addresses a file by integer `file_id` — so **URL-encoding never arises at all**, and the whole cost is one `CodeFile` id→path lookup, since `read_neighborhood` takes a path. Options (b) `?file=<path>` and (c) `{path:path}` are both REJECTED: (b) invents a convention this router does not have and needs `/` percent-encoded; (c) collides ambiguously with the existing `files/{file_id}` route. **The fiddly part of this decision is the part that was designed out, not solved.** **ANSWER 2 — DENOMINATOR: Option B. The UI endpoint computes the denominator ITSELF, via its own separate query.** **`read_neighborhood`, `_describe`, `_apply_budget` and the MCP payload are NOT to be widened, and there is to be NO `include_sizes` flag.** Two reasons, and the first is the load-bearing one. **(1) Widening the payload would silently RE-PRICE the 219.7x Phase 6 figure**, which was measured against the current payload shape — a UI convenience would move the number the whole phase exists to report, and it would move it invisibly, in the direction of looking worse, with no test able to notice. **(2) Different consumers, different needs: the MCP tool's need did not change.** An `include_sizes` flag was considered and rejected on that ground — it is a second shape for one payload, kept alive for a consumer that never asked, and every future reader of the budget accounting would have to establish which shape produced a given figure. **The endpoint owns its own denominator; the tool keeps its own accounting; neither reaches into the other.** **RECORDED ALONGSIDE, because it WILL read as a bug the first time someone notices it: `/files/{file_id}/neighbors` AND `/files/{file_id}/context` ANSWER DIFFERENT QUESTIONS AND WILL RETURN DIFFERENT NEIGHBOUR COUNTS FOR THE SAME FILE, BY DESIGN.** `neighbors` is one hop, capped at `NEIGHBORS_ENDPOINT_CAP = 100`, serving the per-file Mermaid export, and reports `*_total_before_cap` as the true unbounded count. `context` goes through `read_neighborhood`: budget-ranked, `MAX_ENRICHED = 25` enriched entries with every remaining PATH kept, bounded by a token budget that sheds metadata before it ever sheds a path. **Same file, two numbers, and neither is wrong** — one answers "what should this diagram draw", the other answers "what must I read to change this safely". **Pinned here so the divergence is a documented design property rather than a defect someone reconciles by making one of them agree with the other** | **DECIDED 2026-09-04. Checkpoint 1 is UNBLOCKED and NOT YET BUILT — records-and-recon only at this point; no endpoint, no tests, no canary** |
| — | **PHASE 8 CHECKPOINT 1b — FOUR DECISIONS, plus STEP 0's calibration result and TWO CORRECTIONS the calibration turned up.** **DECISION — VERIFICATION FILE: `superset/models/core.py`, `file_id` 2256**, with `utils/core.py` (2419) as a secondary non-load-bearing check. **§17.16 — THE BRIEF WAS WRONG AND IS MARKED, NOT QUIETLY FIXED: the checkpoint-1 brief attributed 258 importers / 22 imports / 51 unresolved to `superset/utils/core.py`. Those are `superset/models/core.py`'s figures.** `utils/core.py` has 11 imports / 346 importers / 78 unresolved / 355 distinct connected. Caught in recon before any code was written against the wrong file. **§17.16 — A SECOND CORRECTION, TO THE BRIEF'S STATED REASON FOR THAT CHOICE, which was that 2256 "is what the Phase 6 219.7x record is anchored to". IT IS NOT.** The 219.7x pooled figure (`phase6-graph-as-context.md` §4.2) is anchored to five files — `scripts/__init__.py`, `commands/annotation_layer/annotation/create.py`, `commands/chart/delete.py`, `superset/__init__.py` and **`superset/utils/core.py`** — and `models/core.py` **appears nowhere in that benchmark.** 2256's 5,062 comes from the checkpoint 4b MCP extension-layer confirmation, a different measurement on a different file with a different instrument (`_estimate_tokens`, not tiktoken). **The DECISION stands — 5,062 is a real, reproducible `_estimate_tokens` value and a sound canary anchor — but the REASON does not, and the two anchors must not be conflated: the benchmark hub is 2419 with a tiktoken graph cost of 5,954.** **DECISION — DENOMINATOR POPULATION: DEDUPED, 274.** A file read once costs once; 280 double-prices the 6 files that both import and are imported by 2256. All three counts are returned under distinct names so the gap self-documents: `connected_files_distinct=274`, `edge_endpoints_total=280`, `overlap_count=6`. **DECISION — A SEPARATE CONSTANT, `_CHARS_PER_TOKEN_SOURCE`.** `_CHARS_PER_TOKEN` (3.6) is calibrated on this repo's **path corpus** and must not be reused for source-code bytes. **DECISION — `view_tokens` = `_estimate_tokens()` over the neighborhood sub-object ONLY, never the endpoint envelope.** **STEP 0 — BRANCH "NO" FIRED. The artifacts are clearly ABSENT, not ambiguous:** `tiktoken` is not installed in `backend/venv` (`ModuleNotFoundError`), no benchmark script or data file exists anywhere in the tree, and `git log --all --diff-filter=AD` finds none ever committed. The only surviving record of checkpoint 3 is the RESULTS TABLE in `phase6-graph-as-context.md` §4.2, which carries per-file *aggregate naive token totals* and **no character counts at all**. So `_CHARS_PER_TOKEN_SOURCE = 3.6` and `calibration_status = "UNVALIDATED_path_calibrated_divisor_applied_to_source_bytes"`, as the NO branch specifies. **THE GAP, RECORDED AS AN OPEN ITEM RATHER THAN HIDDEN IN A COMMENT — AND IT IS QUANTIFIED, because the char side is recoverable from `code_files.size_bytes` even though it was never stored with the benchmark.** Combining §4.2's tiktoken naive totals with summed `size_bytes` over each file's DISTINCT connected set yields **Superset Python source at 4.6737 chars/token pooled (16,059,975 bytes / 3,436,264 tokens), per-file range 4.511–4.907.** Two independent validations that this derivation is sound: the DISTINCT connected counts reproduce §4.2's recorded counts **exactly** for all five files (0, 6, 10, 524, 355), and the summed naive total reproduces §4.2's pooled 3,436,264 exactly. **CONSEQUENCE, stated plainly: 3.6 is roughly 30% BELOW the real figure, which INFLATES the denominator and therefore INFLATES the claimed saving — the opposite of the conservative direction the brief asked for.** On 2256 the difference is `connected_files_tokens` 1,769,307 @3.6 versus 1,355,214 @4.7, i.e. a claimed ratio of **349.5x versus 267.7x**. **The NO branch was followed as written rather than substituted for a judgement call, and the constant is one line to change; the number is recorded here so the change is a decision made against evidence rather than a discovery made later by someone reading a shipped ratio.** | **DECISIONS RECORDED 2026-09-04. `_CHARS_PER_TOKEN_SOURCE = 3.6`, UNVALIDATED. OPEN ITEM: re-set it to a conservative round-up of the derived 4.6737 — a one-line change that lowers the claimed saving on 2256 from 349.5x to ~267.7x** |
| — | **PHASE 8 CHECKPOINT 1b — BUILT AND VERIFIED. `GET /{repo_id}/files/{file_id}/context`, `repos.py:get_file_context`, 9 tests, suite 1,466 -> 1,475 passed / 1 skipped / 0 failed (149s).** Calls the EXISTING `read_neighborhood` (`repos.py:701`) and returns its result under `neighborhood` **unmodified** — asserted by a test that re-invokes the function directly and compares the whole object. `read_neighborhood`, `_describe`, `_apply_budget` and the MCP payload are untouched; the endpoint runs its own DISTINCT edge query for the denominator (Option B). **EVERY PRE-REGISTERED EXPECTATION FOR 2256 HIT EXACTLY, zero deviations:** distinct 274, edge endpoints 280, overlap 6, unresolved 51, bytes 6,369,507, connected tokens 1,769,307, `view_tokens` 5,062, ratio 349.5x, snapshot `a05a0999877f`. Secondary 2419: distinct 355, bytes 8,039,001, `view_tokens` 6,380 (inside the pre-registered 5,000–7,000). **FOUR CANARIES, EACH OBSERVED FAILING FIRST (§15.1):** C1 population via two INDEPENDENT routes — SQL edge query vs the payload's own paths — 274 == 274, broken by restricting the payload route to the `MAX_ENRICHED=25` subset (42 vs 274, under-pricing 84.7%); C2 dedupe, broken at the summed 280; C3 view cost, broken by pricing the envelope (5,228, a **+166 drift that looks entirely plausible**); C4 unresolved exclusion, 51 excluded. **C4's FIRST BREAK ATTEMPT WAS VACUOUS AND IS RECORDED AS SUCH RATHER THAN COUNTED: deleting `AND ci.to_file_id IS NOT NULL` changed nothing (274 -> 274)**, because the join is `ON nb.id = ...to_file_id` and NULL matches no row — the clause is redundant and the guard is STRUCTURAL, so a canary built on removing it could not fail and would have certified nothing. The discriminating break adds the unresolved rows as zero-byte members: **population 274 -> 325 while bytes stay 6,369,507** — the count inflates and the denominator does not, which is the failure C4 actually names. **A REAL BUG CAUGHT ON THE FIRST DIRECT CALL:** `budget: int = Query(DEFAULT_BUDGET_TOKENS)` passed the Query MARKER, not an int, into `read_neighborhood`, which died in `json.dumps`. Changed to a plain default. This repo already documents the trap at `_as_list` (`repos.py:379`) — **the same lesson, in the same file, cost a cycle anyway** | **CLOSED 2026-09-04 — endpoint only. No frontend, no component, no token UI (checkpoints 2–4). OPEN: the `_CHARS_PER_TOKEN_SOURCE` calibration gap recorded in the row above** |
| — | **PHASE 8 CHECKPOINT 1c — THE SOURCE DIVISOR IS RECALIBRATED TO 4.7 AND VALIDATED AGAINST MEASURED GROUND TRUTH. Suite 1,475 -> 1,479 passed / 1 skipped / 0 failed.** **§17.16 — STEP 0's "NO" BRANCH WAS FOLLOWED CORRECTLY AND IS NOW SUPERSEDED, and the distinction matters.** The branch instruction assumed that if no artifact stored per-file char counts, no calibration was available. That assumption was wrong rather than the branch being mis-executed: **the char side was recoverable from `code_files.size_bytes` even though it was never stored with the benchmark**, so pairing it with §4.2's naive tiktoken totals yielded 4.6737 pooled. `_CHARS_PER_TOKEN_SOURCE` moves **3.6 -> 4.7**, a conservative ROUND-UP (higher divisor -> smaller denominator -> smaller claimed saving). The old 3.6 was ~30% low and inflated the denominator in the flattering direction: on 2256, **349.5x becomes 267.7x**. **`calibration_status` replaces `UNVALIDATED...` with `derived_from_phase6_4.2_aggregate_tiktoken_cl100k_at_a05a0999_rounded_conservative`. WHAT THAT LABEL DOES NOT ASSERT, recorded because a label is a claim:** it is **AGGREGATE-level**, derived from per-file totals over connected sets and never from a per-file char/token pair, so it must not be quoted as "this file tokenises at 4.7"; it covers **Superset Python source only**; and it makes **no claim for any other language or repository** — applying it to a JS or Go repo would repeat exactly the mistake that reusing the path-calibrated `_CHARS_PER_TOKEN` here would have been. **VALIDATED ON THE ONE FILE BOTH INSTRUMENTS MEASURED.** `superset/utils/core.py` (2419) is a checkpoint-3 benchmark hub, so it carries a tiktoken naive cost AND a graph cost — the only place this endpoint's estimate can be checked against measurement rather than against itself. **All three directional assertions hold, and the direction is the point, not the closeness:** denominator 1,710,425 **<** measured naive 1,746,672 (2.08% apart); `view_tokens` 6,380 **>** measured graph 5,954 (7.15%); ratio 268.09 **<** measured 293.36 (8.61%). All within 10%. **CANARIED at the superseded 3.6: the denominator goes to 1.2785x measured and the ratio to 1.1931x — both cross 1.0 and both fail.** `view_tokens` correctly does NOT move under that break, being priced by `_estimate_tokens` rather than by this divisor. **A POPULATION CAVEAT THAT THE PASSING ASSERTION COULD HAVE HIDDEN:** §4.2's naive cost INCLUDES the file's own text (76,119 B) and `connected_bytes` does not, so the comparison is not perfectly like-for-like. **Checked both ways — 1,710,425 excluding self and 1,726,621 including it, both under the measured 1,746,672 — so the result is not an artifact of the population difference, but the true margin is 1.1% rather than 2.1%.** **NEW PAYLOAD FIELD `estimator_vs_measured` = 0.914 for 2419, `null` everywhere else, deliberately.** Below 1.0 means the estimate lands under measured ground truth, which is what the round-up was for. It is not computed by analogy for unbenchmarked files: a field that silently means something different per file is worse than an absent one (§17.25) | **CLOSED 2026-09-04. Divisor 4.7, validated, estimate lands 8.6% BELOW measured. ck4 may put a ratio on screen. Residual: validated on ONE file; the 4.511–4.907 per-file spread is not narrowed by this** |
| — | **PHASE 8 CHECKPOINT 1d — `connected_index` ADDED TO THE ENVELOPE, plus the per-file divisor band recorded as BINDING ON CHECKPOINT 4's LABELING. Suite 1,479 -> 1,485 passed / 1 skipped / 0 failed.** **`connected_index: [{id, path}]`, an ENVELOPE sibling — `neighborhood` stays byte-identical to what `read_neighborhood` returns.** Rationale: the panel is self-navigating by node click, but graph nodes carry PATHS while this endpoint and the URL take an integer id, so without the map every click costs a resolve roundtrip. **The ids were already selected by the denominator query and were being discarded — this is the same query, one column wider, at zero additional cost.** **C3's BOUNDARY GOT ITS FIRST LIVE TEST AND HELD.** Adding an envelope sibling is precisely the change that would move `view_tokens` if it priced the envelope rather than the neighbourhood sub-object. It did not move: **5,062 and 6,380 unchanged**, `connected_files_tokens` and `saved_ratio` unchanged (1,355,214 / 267.72 and 1,710,425 / 268.09), while `_estimate_tokens(envelope)` demonstrably grew. Pinning that boundary at checkpoint 1b was not ceremony; this is the change it was pinned against. Canaried: the id-set assertion broken by dropping one entry (273 vs 274). **THE PER-FILE DIVISOR BAND, RECORDED AS A CONSTRAINT ON CHECKPOINT 4 RATHER THAN AS A CHANGE TO THE CONSTANT.** Sorted by connected-set size, the derived divisors are **4.907 (6 connected) · 4.872 (10) · 4.698 (524) · 4.646 (355)**; the 4.511 datapoint is DISCARDED as noise (a 785-byte file). **The band is size-dependent and 4.7 sits at the large-neighbourhood end**, so on small-neighbourhood files 4.7 overstates the denominator by up to **4.4%** (4.907/4.7) — the NON-conservative direction, and the mirror image of the error 1c fixed. **THE CONSTANT DOES NOT CHANGE: 4.7 is validated where it matters** (the hubs, where the saving is claimed and where the band bottoms out at 4.646–4.698). **BINDING ON CHECKPOINT 4: a per-file ratio on a small-neighbourhood file carries a ~4% upward bias and must not be labelled with the confidence the hub-validated aggregate earns.** Recorded now because the temptation at ck4 will be to print one number with one label for every file, and the honest label differs by neighbourhood size | **CLOSED 2026-09-04. Endpoint complete for ck2–ck4. OPEN and binding on ck4: the size-dependent divisor band (~4.4% upward bias on small neighbourhoods)** |
| — | **PHASE 8 CHECKPOINT 2 — FIVE DECISIONS (D9–D13), plus V1's answer and a §17.16 note on the handoff brief's frontend claims.** **D9 — THE URL CARRIES AN INTEGER `file_id`.** Cold-load needs no resolution: `?file=<path>` has nothing to resolve against before the first fetch, and translating one path through `/ranking` costs **2.85 MB** for Superset. **The UI displays the PATH regardless** — the endpoint returns it, so the URL's encoding is an implementation detail the user never sees. **OPEN ITEM, recorded now rather than discovered later: `code_files.id` is not stable across re-ingest.** If ids are reassigned, previously shared links do not 404 cleanly — they resolve to a DIFFERENT file, or surface as a 409 if that id fell out of the snapshot. A link is only as durable as the ingest it was made against. **D10 — THE LOADER DISCRIMINATES STATUS CODES; `loadRanking`'s `.catch(() => setNull)` IS NOT COPIED.** That shape flattens every failure into one state, which for `/context` merges 409 "in code_files but not in the graph snapshot" with "no file selected" — two states a user must be able to tell apart, since one is fixed by re-ingesting and the other by clicking a file. Three distinct UI states are required. **D11 — `keyedOnFiles = FALSE` for Context.** The flag's own documented test is "could a file filter meaningfully apply to what this view renders", and a single-file view answers no. **Losing the DetailPanel is INTENDED — Context is its own detail surface.** The flag is NOT split: it conflates the filter bar and the panel, but splitting it on speculation is the wrong order. If ck3 needs the panel, split then, with evidence. **D12 — THE SOURCE VIEWER LEAVES PHASE 8 DEMO SCOPE, to Phase 8.5.** `CodeSnippet.tsx` does not exist and no endpoint serves file contents, so it needs a new endpoint AND a new component, 1–2d. **The decisive argument is not cost, it is that it would ship DEAD: Superset's `repos.local_path` is `C:\Users\anshuman.r\...` with `exists=False` on this Linux VM, so a source endpoint would return nothing for the demo repo.** ck4 becomes token badges only. **ck3 KEEPS its full 4–5d** — the saving is not reallocated. **D13 — the `/ranking` `limit` param is ck3's first task, not ck2's.** **V1 — ANSWERED, AND THE MINIMAL SHAPE IS CONFIRMED.** `scopeGraph` and `buildGraphElements` both take `ScopeNode[]` = `{id: number; path: string; subsystem_modularity_id?: number-or-null}` (`dependencyGraphScope.ts:27-31`, `dependencyGraphElements.ts:76-84`) — exactly the minimal shape, with the subsystem id OPTIONAL. **ck3's 4–5d estimate stands. Two gaps recorded so ck3 does not meet them cold:** `connected_index` carries no `subsystem_modularity_id`, so compound-folder cluster colouring (`dominantCluster`) degrades to unclustered unless ck3 adds it; and **`connected_index` supplies NODES but not EDGES** — `buildGraphElements` needs `ScopeEdge[]` = `{source, target}` as id pairs, which the payload does not contain. For the 1-hop star Context actually draws (target ↔ each connected file) they are derivable from `connected_index` alone; for 2+ hops, or edges AMONG neighbours, they are not. **§17.16 — THE HANDOFF BRIEF ASSERTED TWO FRONTEND FACTS THAT DO NOT EXIST, and the pattern is worth naming.** (1) **`CodeSnippet.tsx`** — described in false DETAIL, including a props signature and a usage site; no file of that name exists anywhere in the repo. (2) **the `?file=` convention** — the complete URL-param set is `clusters, hideNoise, hideZeroFanIn, languages, page, q, scorer, segments, subsystem, subsystemAlgo, view`; there is no `file` param in any view, and file selection is React state only (`RepoDetail.tsx:340`). **EVERY BACKEND CLAIM FROM THE SAME BRIEF HAS HELD; every frontend claim checked so far has not.** The asymmetry is the finding: **frontend claims from that brief are to be VERIFIED BEFORE BEING BUILT ON**, and the detail in which a false claim was stated is not evidence for it — the invented props signature was the most specific thing said and the most wrong | **DECISIONS RECORDED 2026-09-04. V1 clear, no stop. ck2 building: data layer + tab only** |
| — | **PHASE 8 CHECKPOINT 2 — BUILT. Data layer + Context tab. Frontend 19 -> 21 test files / 272 tests; backend UNCHANGED at 1,485 passed / 1 skipped / 0 failed, as required.** `api.ts` gains `ApiError` (message unchanged, `.status` added) and `fetchContext`; `lib/contextLoad.ts` holds the status discrimination with an injected async dep; `RepoDetail.tsx` gains the `context` ViewT, `keyedOnFiles: false`, a `?fileId=` integer param and a definition-list render — raw labeled values only, no chart, no badge, no ratio framing. **THE D7 TRIPWIRE IS REAL AND WAS CANARIED AGAINST THE ACTUAL FILE, not a synthetic one:** injecting `connected_files_tokens - view_tokens` and a literal `4.7` into `RepoDetail.tsx` failed BOTH assertions; reverted, `tsc --noEmit` clean, green again. Its limits are stated in-file as `elkLayoutRun.wiring.test.ts` states its own — text not AST, two files not the repo, proves no arithmetic is WRITTEN rather than that the displayed number is right. It also carries its own canary, so the regexes are proven able to fail. **409 IS UNREACHABLE END-TO-END, AND THIS IS REPORTED RATHER THAN SIMULATED.** `read_repo_graph` is `SELECT ... FROM code_files WHERE repo_id = :r` with **no filter** (`graph_read.py:279-284`), so every `code_files` row becomes a graph node — verified on repo 6: **6,584 rows, 6,584 nodes, 0 paths missing**. The endpoint 404s unless the row exists in `code_files`, and every such row is in the graph, so `read_neighborhood`'s `ValueError` cannot fire through this route. **The 409 handler is therefore defensive dead code today**, and the D10 discrimination is proven only at the lib layer with an injected `ApiError(409)`. **D10 IS STILL RIGHT — the three states are distinguishable and tested — but the claim "we handle 409" is NOT end-to-end evidenced, and recording it as though it were would be the §17.30 shape.** If a future change makes the graph a filtered subset of `code_files`, the branch becomes live and the test already covers it. **NO BROWSER VERIFICATION WAS POSSIBLE and that is not glossed:** `:8000` holds the port (`app.main`'s guard refuses to start beside it) but does not answer HTTP — `curl /docs` times out. The rendered values reported were produced by calling the endpoint function and formatting the exact 18 rows the `dl` emits, in source order. **That is the data arriving, not the tab observed in a browser** | **CLOSED 2026-09-04 — ck2 scope only. OPEN: 409 unreachable (defensive branch, lib-tested only); no browser verification; D9's id-instability-across-re-ingest** |
| — | **PHASE 8 CHECKPOINT 3a — D9 CORRECTED, D14 and D15 recorded, and the `:8000` wedge diagnosed as a REAL BUG in the port guard.** **§17.16 — D9's OPEN ITEM WAS WRONG AND IS MARKED, NOT REWRITTEN.** It said a stale shared link "surfaces as a 409". **The ck2 409 proof falsifies that:** `read_repo_graph` has no filter, so every `code_files` row is a graph node and the 409 branch is unreachable. **The real failure is worse than the one recorded.** With an id in the URL, a reassigned id renders a DIFFERENT file's neighbourhood — **correctly computed, plausible, and with no error at all.** Wrong file, right-looking numbers, undetectable by the consumer: §17.25 exactly. The original entry predicted a loud failure where the truth is a silent one, which is the more dangerous direction to be wrong in. **D14 — THE URL CARRIES A PATH FINGERPRINT ALONGSIDE THE ID** (`?fileId=2256&fp=<short hash of path>`). On load the UI compares `fp` against the `path` the endpoint returns; a mismatch renders an explicit **"this link points at a file that has changed"** state and **never** substitutes silently. **No backend change is needed: `path` is already in the envelope** (ck1b), so the fingerprint is computable client-side from data already returned. The UI check is ck3b. **D15 — ALL NODE COLOURING IN THE CONTEXT VIEW COMES FROM `connected_index`, NEVER from `neighborhood`'s `_describe` `cluster` field. BINDING ON ck3b.** `cluster` covers only the `MAX_ENRICHED = 25` entries; mixing it with a widened `connected_index` for the other 249 would be **two instruments for one visual property (§17.28)**, and the discrepancy would surface as a mis-coloured node with no way to trace which source produced it. One source for one property. **THE `:8000` WEDGE — IDENTIFIED, AND THE PORT GUARD IS SELF-DEFEATING.** The listener was `./venv/bin/python run.py` (pid 165068, user `hack-t36`, cwd `backend/`, 2h48m old), launched **detached by a PRIOR Claude Code session** via `setsid nohup`. It held the socket but answered nothing — `Recv-Q` 32, `curl /docs` timing out. **CAUSE:** `run.py` runs `uvicorn.run(..., reload=True)`, where the **RELOADER PARENT** binds and holds the port and the worker inherits it. `app/main.py`'s `_fail_loudly_if_port_already_bound` then probes with `connect_ex` on every worker import, finds the parent's own socket answering, and raises — **so every reload after the first kills the new worker while the parent keeps the port open.** The result is the worst available failure shape: a port that LISTENS and never replies, which looks like a hung server rather than a dead one. **THE GUARD'S OWN COMMENT ASSERTS THE OPPOSITE AND IS FALSE:** "A normal `--reload` restart is unaffected -- that reloader kills the old worker and waits for it to exit before spawning a new one, so the port is genuinely free by the time this check runs again." It is not free; the parent holds it. **22 occurrences of "already answering" in `.devlogs/backend.log`**, the first triggered by `_mcp_utf8_canary_tmp.py` — so this has been broken since an earlier session, and every backend edit since has silently re-killed the worker. **A comment claiming a property the code does not have, where the failure is invisible because the symptom (a hanging request) is indistinguishable from load.** Stale process cleared; the vite dev server on 5173 was left running. **NOT FIXED HERE — out of ck3a scope, and it needs a decision: drop the guard under reload, or detect the reloader parent rather than the port** | **RECORDED 2026-09-04. D9 corrected, D14/D15 pinned. OPEN and blocking browser verification across checkpoints: the reload/port-guard conflict in `app/main.py:35-45` vs `run.py:34`** |
| — | **PHASE 8 CHECKPOINT 3a — BUILT. `connected_index` widened, `unresolved_edges` added, backend live. Backend suite 1,485 -> 1,494 passed / 1 skipped / 0 failed; frontend UNCHANGED at 21 files / 272 tests.** **THE SAME Q2 QUERY, WIDENED — `neighborhood`, `_describe`, `_apply_budget` and the MCP payload untouched.** `connected_index` entries gain `direction` and `subsystem_modularity_id`; direction is aggregated **per NEIGHBOUR via GROUP BY, not per edge**, which is what makes a file appearing on both sides one row labelled `both` instead of two contradictory rows. **DIRECTION RECONCILES EXACTLY: 2256 = 22 + 258 − 6 = 274; 2419 = 11 + 346 − 2 = 355.** Cross-checked against `read_neighborhood`'s own `imports.total` / `importers.total` — two independent routes to the same two numbers, as C1 does for population. **THE DISCRIMINATING BREAK MATTERS HERE and was run: swapping `from_file_id`/`to_file_id` in the CASE branch INVERTS the split (2256 -> 258/22, 2419 -> 346/11) while leaving the TOTAL at 274/355 and `both` at 6/2 untouched — so a length-only or total-only assertion passes the swap clean.** That is why the reconciliation is asserted per side. **C3's SECOND LIVE TEST PASSED, with much larger additions than ck1d's:** two new keys on every one of 274/355 entries plus a whole new 51/78-element array, and **`view_tokens` stayed at 5,062 / 6,380**, with `connected_files_tokens`, `saved_ratio` and `connected_index` length all unchanged. **The envelope/sub-object boundary has now held across three separate widenings.** **SUBSYSTEM COVERAGE: 274 of 274 and 355 of 355 — 100% non-null on both files.** Reported as measured, with no target asserted. **ck3b can colour every node from `connected_index` alone (D15) with no null-handling path exercised on these two files** — but 100% here is a property of repo 6 after subsystem computation, NOT a guarantee, so ck3b still needs a null branch it cannot test against this fixture. **`unresolved_edges` IS DISPLAY DATA AND IS RECORDED AS NEVER PRICED**, in a comment above its own query as well as here: 51/78 entries, length equal to `unresolved_excluded`, no specifier reaching `connected_index`, and `priced_files == connected_files_distinct` unaffected by their presence. Folding them into the denominator would inflate the count while `connected_bytes` stayed put — the C4 failure arriving through a new field. **BACKEND NOW LIVE AND ANSWERING over real HTTP** — `/docs` 200 in 0.9ms, and `/api/repos/6/files/2256/context` returns **401**, which is the auth gate rather than a 404 and so confirms the route is registered and reachable. **First real-HTTP confirmation of this endpoint in the project.** **WARNING, and it follows directly from the guard bug recorded above: the reloader parent (188994) and worker (188996) BOTH hold fd 3 on the port, so the NEXT backend edit will trigger a reload whose worker the port guard kills again.** Until `app/main.py:35-45` vs `run.py:34` is resolved, a live backend and backend editing are mutually exclusive on this machine | **CLOSED 2026-09-04 — ck3a scope only, no component. Backend live for ck3b. OPEN: the reload/port-guard conflict; ck3b needs an untestable null-subsystem branch** |
| — | **PHASE 8 CHECKPOINT 3a-bis — THE PORT GUARD IS MOVED, NOT DELETED, and the reload conflict is closed. Backend suite 1,494 passed / 1 skipped / 0 failed **WITH THE SERVER LIVE** — a combination the old placement made impossible. Frontend untouched at 21 files / 272 tests.** **§17.30 — A COMMENT THAT ASSERTED THE OPPOSITE OF WHAT THE CODE DID.** `_fail_loudly_if_port_already_bound` was never wrong; it was **in the wrong PROCESS.** Under `uvicorn.run(reload=True)` the RELOADER PARENT binds the listening socket and the worker inherits fd 3, so a `connect_ex` probe at worker-import time is **structurally guaranteed** to find its own parent answering and kill the worker it exists to protect — while the parent keeps the port, producing a listener that never replies. **The old comment stated: "A normal `--reload` restart is unaffected -- that reloader kills the old worker and waits for it to exit before spawning a new one, so the port is genuinely free by the time this check runs again." It is not free; the parent holds it.** 22 `already answering` entries had accumulated in `.devlogs/backend.log` before this session, the first from `_mcp_utf8_canary_tmp.py`, **so every backend edit for days silently re-killed the worker and each instance presented as a hanging request rather than a crash.** **THE MISPLACEMENT HAD ALREADY ANNOUNCED ITSELF ONCE AND WAS PATCHED INSTEAD OF DIAGNOSED.** The guard carried `if "pytest" not in sys.modules:` because otherwise it failed every TestClient test whenever a dev server was running — and `tests/test_graph_wire_format.py:35` does `from app.main import app`. **A check that has to be excluded from one caller after another is answering its question in the wrong place.** The exclusion is now unnecessary and is gone with it. **PRE-CHANGE AUDIT, per instruction: nothing depended on the guard firing at import.** Sole non-test importer of `app.main` is `run.py:34`; no test or conftest references the guard; no gunicorn/Procfile/ASGI entrypoint exists. **THE FIX:** the probe and its function move verbatim into `run.py`, called ONCE in the parent before `uvicorn.run` binds anything; `app/main.py` loses the call, the function and its now-unused `import socket`, and keeps a marked comment saying where it went and not to bring it back. **Deliberately NOT special-cased on reload and NOT detecting the parent via `getppid` or uvicorn internals** — a guard whose correctness depends on undocumented process topology is a fragile instrument guarding a fragile failure, which this record has enough instances of already. **THREE CANARIES, AND THE THIRD IS THE ONE THAT MATTERS.** (1) Cold start on a free port: `Uvicorn running`, `Application startup complete`, `/docs` **200 in 1.1ms**, zero `already answering`. (2) Reload survival: editing `app/main.py` and then `run.py` fired two real reloads — **worker pid changed 188996 -> 189197**, both reached startup-complete, `/docs` still 200, and **zero `already answering` in the new log**. (3) **DISCRIMINATING — a second instance launched against the live server still fails loudly**, with the identical message and `exit 1`, raised in `run.py:72` **before** `uvicorn.run`, so no half-started server is left behind. **Canaries 1 and 2 are both satisfied by simply deleting the guard; only (3) distinguishes a fix from a removal.** **FIRST AUTHENTICATED END-TO-END HTTP CONFIRMATION OF THE `/context` ENDPOINT: HTTP 200 in 0.51s**, `view_tokens` 5,062 · `connected_files_distinct` 274 · `saved_ratio` 267.72 · `connected_index` 274 · `unresolved_edges` 51 · snapshot `a05a0999877f` — matching ck3a's in-process figures exactly. Token minted via `create_access_token(1)` rather than registering a user, so **no row was written to the live database** | **CLOSED 2026-09-04. Blocker cleared: a live backend and backend editing now coexist. Not Phase 8 feature scope** |
| — | **PHASE 8 CHECKPOINT 3b-1 — D16 and D17, the two decisions that keep the DRAWING reconcilable to the NUMBERS.** Both are about the same failure: a graph and a badge that are each internally consistent and disagree with one another, where the viewer has no way to tell which is right. **D16 — A "both"-DIRECTION FILE IS ONE NODE, MARKED, POSITIONED ON THE `importedBy` SIDE.** The dominant relation decides the side (252 importedBy vs 16 imports on 2256). **Rendering it twice would put 280 objects on screen under a badge reading 274, and someone will count.** The marking carries the both-ness; the node count stays reconcilable to `connected_files_distinct`. **The EDGE count is the opposite decision and deliberately so: a "both" file contributes TWO edges, so edges total 280 = `edge_endpoints_total`.** Nodes reconcile to 274, edges reconcile to 280, and the 6-file gap between them is exactly `overlap_count` — three numbers that check each other rather than one that has to be trusted. **D17 — UNRESOLVED SPECIFIERS ARE NOT `ScopeNode`s AND MUST NOT SHARE THE FILE VISUAL GRAMMAR.** They have no `code_files` row, no size, and are deliberately outside the priced population — `builtins` and `_thread` are not files. **If they render as file-like nodes the graph shows 274 + 51 = 325 objects while every payload number says 274: consistent on both sides, wrong, and undetectable by the viewer (§17.25).** They therefore return through a **SEPARATE CHANNEL** from the adapter, so the type system enforces the separation rather than a convention doing it. **BINDING ON ck3b-2: a distinct visual class — different shape, or a separate labelled tray — NEVER merely a different colour.** A colour difference is a legend lookup; a shape difference is immediate, and this distinction has to survive someone glancing at the graph. **Per D15 the adapter reads `subsystem_modularity_id` ONLY from `connected_index`, never from `neighborhood`'s `_describe` `cluster` field** — that field covers just the 25 enriched entries, and mixing the two would be two instruments for one visual property | **RECORDED 2026-09-04. Nodes reconcile to 274/355, edges to 280/357, gap == overlap_count. ck3b-2 owes unresolved a distinct SHAPE** |
| — | **PHASE 8 CHECKPOINT 3b-1 — BUILT. `src/lib/contextGraph.ts`, pure and headless, 26 tests. Frontend 21 -> 22 files / 272 -> 298 tests; backend UNCHANGED at 1,494 passed / 1 skipped / 0 failed.** `buildContextGraph(envelope)` returns `nodes: ScopeNode[]` · `edges: ScopeEdge[]` · `unresolved: UnresolvedT[]` (separate channel) · `directionOf: Map` · `centreId` · `counts`. Conforms to `dependencyGraphScope.ts:27-31` and `dependencyGraphElements.ts:76-84` **without widening either contract** — asserted per node, so a future field addition fails rather than silently passing through cytoscape. No fetch, no React, no cytoscape, no ELK; purity and determinism are both tested. **THE THREE-WAY RECONCILIATION IS THE DELIVERABLE, not the node list.** `nodes` = `connected_files_distinct` + 1 centre (**275 / 356**); `edges` = `edge_endpoints_total` (**280 / 357**); and **`edges − connected == overlap_count` (6 / 2)**. Three payload-pinned numbers that check each other, so an adapter that collapsed or duplicated the both-direction files breaks an identity rather than producing a plausible graph. Edge orientation follows the real relation — 22 out of centre, 258 into it on 2256 — and every edge touches the centre, making the 1-hop star explicit by construction. **EACH BREAK WAS RUN AGAINST THE REAL ADAPTER, not asserted arithmetically in-suite.** (1) Collapsing `both` to one edge: **6 tests fail** across both files (edge count, the third identity, orientation). (2) Rendering `both` twice: **8 tests fail** — node count, the `connected_files_distinct` reconciliation, unique-ids, AND the D17 absence check, which is the interesting one: duplicating nodes trips a guard aimed at a different defect. (3) Routing unresolved into `nodes`: **8 tests fail**, 326 objects where every payload number says 274. Reverted after each; green at 26 and `tsc --noEmit` clean. **FIXTURES ARE REAL, CAPTURED OVER AUTHENTICATED HTTP** from the live backend at snapshot `a05a0999877f` (54KB / 72KB) rather than hand-written — a hand-built fixture would only prove the adapter agrees with my idea of the payload. **THE NULL-SUBSYSTEM BRANCH IS SYNTHETIC-ONLY AND SAYS SO IN THE TEST FILE IN CAPITALS.** Repo 6 is 100% covered (274/274, 355/355 at ck3a), so **no real envelope reaches that path** and the fixture is hand-edited. Green there means "null passes through without crashing", NOT "null subsystems are verified on data". The two assertions worth having are that a null node is **not dropped** (which would make the graph quietly incomplete while disagreeing with `connected_files_distinct`) and that null **does not become 0**, which would collide with a real subsystem id. **ck3b-2's colour handling for null will be equally unverified until a repo without computed subsystems is ingested** | **CLOSED 2026-09-04 — adapter only, no component. OPEN: null-subsystem path unexercised on real data; ck3b-2 owes unresolved a distinct SHAPE per D17** |
| — | **PHASE 8 CHECKPOINT 3a-ter — D18: THE DENOMINATOR INCLUDES THE CENTRE FILE. §17.16 correction to a decision taken on a MISREAD OF MY OWN REPORT.** **WHAT WENT WRONG, and it is a reporting failure before it is a code one.** At ck1c I reported the exclude-self population difference as a **1.1% margin** on `superset/utils/core.py` and treated it as a footnote. **That figure is scale-dependent and I presented it as if it characterised the divergence.** It does not: 1.1% is the margin on a 355-connection hub, and **at the floor the divergence is TOTAL.** For a zero-connection file the benchmark's denominator is the file's own text while ours is **0** — 100% divergence, and precisely at the point where the honest 0.93x–293x spread bottoms out, which is the one number the whole distribution argument rests on. **A single-file margin quoted as a property of the method: §17.5c, and the §17.28 drift it licensed was signed off on the strength of it.** **D18 — `connected_bytes` and `connected_files_tokens` NOW INCLUDE THE CENTRE FILE'S OWN `size_bytes`.** Two reasons. (1) It restores **one instrument for one number**: the endpoint now measures the same quantity the 219.7x benchmark measured (§4.2's naive cost = the file plus every file directly connected to it), so the comparison is like-for-like instead of nearly-so. (2) The counterfactual a user is actually being offered is **"read this file and its connected files"** — a denominator that silently omits the file you are about to open is answering a question nobody asked. **THE ASYMMETRY IS DELIBERATE AND IS NAMED IN THE FIELD COMMENTS, because a reader must not have to guess: `connected_files_distinct` EXCLUDES self (274/355) while `connected_bytes` INCLUDES it.** The count stays the connected count because **ck3b-1's node/edge/overlap reconciliation is pinned to it** — nodes = `connected_files_distinct` + 1, edges = `edge_endpoints_total`, gap = `overlap_count`. Folding the centre into the count would break all three identities to tidy one field name. | **RECORDED 2026-09-04. Denominator scope corrected; count and bytes deliberately differ in scope, stated rather than inferred** |
| — | **PHASE 8 CHECKPOINT 3a-ter — BUILT. D18 applied; the honest floor CALLED FOR THE FIRST TIME and it deviates from prediction. Backend 1,494 -> 1,504 passed / 1 skipped / 0 failed; frontend UNCHANGED at 22 files / 298 tests.** **2419: `connected_bytes` 8,115,120 · `connected_files_tokens` 1,726,621 · `saved_ratio` 270.63 · `estimator_vs_measured` 0.923 (raw 0.92253, the field rounds to 3dp — 0.9225 is NOT representable in the payload as shipped).** 2256: own 63,868 · bytes 6,433,375 · tokens 1,368,803 · ratio 270.41. **THE FLOOR FILE — `scripts/__init__.py` (id 1107, 785 bytes, ZERO connections), and it MISSES THE PREDICTED BAND.** Reported not reconciled: distinct 0 · endpoints 0 · `connected_bytes` **785** (was 0 before D18) · tokens **167** · view **168** · **`saved_tokens` −1** · **`saved_ratio` 0.9940**. **Predicted ~0.89x, band 0.80–0.95; actual 0.994 — outside it.** **WHY, DECOMPOSED, AND IT INDICTS A DECISION I MADE AT ck1d.** The benchmark measures this file at 174 naive / 188 graph = **0.9255x**. Ours is 167/168 = 0.9940x. Two divergences, both in the same direction: **(1) the divisor.** This file's own implied rate is 785/174 = **4.511 chars/token — which is exactly the datapoint discarded as "noise, 785-byte file" at ck1d.** At the floor the discarded datapoint is the GOVERNING one, and 4.7 makes the denominator 4.0% low. **(2) the view side.** `_estimate_tokens` gives 168 where tiktoken gives 188, 10.6% low. **Net effect: our ratio sits CLOSER TO 1.0 than the measured truth, i.e. it UNDERSTATES the loss at the floor (−1 tokens against a true −14).** That is the NON-conservative direction — on the one number Phase 6 reports specifically to be honest about. **ck1d recorded the band as "binding on ck4's labeling" and predicted a ~4.4% upward bias on small neighbourhoods; this is that bias arriving, at the worst possible file, compounded by a second instrument divergence nobody had measured because THE ENDPOINT HAD NEVER BEEN CALLED ON A ZERO-CONNECTION FILE.** The constant is NOT changed here — that is a ck4 labelling decision with a real trade (4.511 is right for the floor and wrong for the hubs) — but the floor's ratio must not be presented with hub-level confidence. **THE SUB-1x RESULT REACHES THE PAYLOAD INTACT, and this was canaried in BOTH directions.** `max(0, ...)` on `saved_tokens`: **3 tests fail** — and critically **`saved_ratio` 0.994 SURVIVES the clamp**, so the sign is hidden while the ratio still looks plausible. That is the §17.25 shape precisely, and it is why the guard asserts the sign and the ratio and their mutual consistency rather than one of the three. `max(1.0, ...)` on `saved_ratio`: **2 tests fail**. A source-level tripwire backs both. **MY FIRST TRIPWIRE WAS WRONG AND IS RECORDED AS SUCH: it banned `or 0`, which flagged `(file.size_bytes or 0)` — a null-coalesce on a nullable column, not a clamp.** Narrowed, with the reason in-file: **a tripwire whose first catch is a false positive teaches people to delete tripwires.** **FIVE PRE-D18 ASSERTIONS WERE SUPERSEDED AND UPDATED WITH §17.16 MARKERS, NOT DELETED** (1,355,214/267.72, 1,710,425/268.09, 0.914 across ck1c/ck1d/ck3a tests) — in each case the PROPERTY under test was unaffected and only the value moved, which is stated in each note. `connected_files_distinct` stays 274/355, and **ck3b-1's 26 adapter tests pass unchanged against re-captured fixtures** | **CLOSED 2026-09-04. Denominator like-for-like with §4.2; floor verified and sub-1x unclamped. OPEN and now SHARPER: the divisor band bites hardest at the floor (0.994 vs measured 0.926) — binding on ck4; and `estimator_vs_measured`'s 3dp rounding** |
| — | **PHASE 8 CHECKPOINT 3a-quater — TWO §17.16 CORRECTIONS TO MY OWN CALLS, and D19: THE DIVISOR STAYS FIXED.** **CORRECTION 1 — DISCARDING THE 4.511 DATAPOINT AT ck1d WAS WRONG TWICE.** It was labelled "noise, 785-byte file" and dropped from the divisor band. **(a) It was a real measurement, not noise** — tiktoken cl100k on a real file at a real snapshot, no less valid than the other four. **(b) It was the datapoint GOVERNING THE FLOOR FILE**, which is the single number Phase 6 reports specifically in order to be honest about where the graph LOSES. **The deeper error is the shape of the reasoning: the four surviving divisors ran high on small connected sets and low on large ones — that is a SIZE TREND, and I removed its small end as an outlier.** Discarding the extreme of a trend because it is extreme is how a trend gets mistaken for scatter. **CORRECTION 2 — MY PRE-REGISTERED INVARIANT "`estimator_vs_measured` MUST STAY < 1.0" IS FALSE.** It was pinned at ck1c and held through ck1c/1d/3a/3a-ter — **because every file I checked was a hub.** At the floor it is **1.074: the estimator OVERSTATES the ratio there.** An invariant that holds on the sample you chose is not an invariant, and this one was checked four times without ever being tested where it could fail (§15.1). It is replaced by a **two-sided bound**, measured across all five §4.2 files rather than asserted. **D19 — `_CHARS_PER_TOKEN_SOURCE` STAYS FIXED AT 4.7. Not size-dependent, not interpolated.** Four reasons, and the last is the one that decides it. (1) Any interpolation is **unvalidated between anchors** — five points, four gaps, and no measurement inside any of them. (2) It would make the denominator **a function of the thing being measured**, so a file's price would depend on its own connectivity and two files of identical size would be priced differently. (3) It would **break every exact-match canary since ck1c** — 5,062 / 1,368,803 / 1,726,621 and the C1–C5 reconciliations — for a correction of at most ~4%. (4) It **invites the exact question the guard structure exists to close**: "which divisor produced this number?" A single labelled constant with a measured error envelope is auditable; a curve is not. **THE ESTIMATOR'S ERROR IS BOUNDED AND LABELLED, NOT TUNED AWAY** — that is the whole disposition of this checkpoint | **RECORDED 2026-09-04. Divisor fixed by decision; the <1.0 invariant retired as false; error envelope to be measured, not assumed** |
| — | **PHASE 8 CHECKPOINT 3a-quater — THE ESTIMATOR'S ERROR ENVELOPE, MEASURED ACROSS ALL FIVE §4.2 FILES. ALL THREE PRE-REGISTERED BANDS MISSED, all in the same direction. Backend 1,504 -> 1,512 passed / 1 skipped / 0 failed; frontend unchanged at 22 files / 298 tests.** Only production change: `estimator_vs_measured` to **4dp** (3dp could not express 0.9225, and being checkable against the benchmark is that field's entire job) and the benchmark table extended from one file to five. **THE TABLE (our/measured, by connected-set size):** 0 conn **1.0740** · 6 conn **1.0613** · 10 conn **1.0910** · 355 conn **0.9225** · 524 conn **0.9721**. Denominator o/m: 0.9598 / 1.0440 / 1.0366 / 0.9885 / 0.9996. View o/m: 0.8936 / 0.9836 / 0.9501 / 1.0715 / 1.0283. **PRE-REGISTERED BANDS, ALL THREE OUTSIDE AND ALL THREE HIGH:** 6 conn predicted 1.02–1.06, actual 1.0613 (+0.0013, marginal); 10 conn predicted 1.01–1.05, actual **1.0910 (+0.0410)**; 524 conn predicted 0.90–0.95, actual **0.9721 (+0.0221)**. **The one-directional miss is itself the finding: the prediction was systematically low, i.e. the estimator is more favourable to us than expected at every size checked.** **ENVELOPE: 0.9225 – 1.0910, inside the 0.90–1.10 stop window but with only +0.0090 of headroom at the top.** Recorded so a future widening is noticed rather than absorbed. **THE CROSSOVER IS A BRACKET, NOT A POINT: between 10 and 355 connected files — 345 wide, with NO measurement inside it.** Below it we OVERSTATE the saving; above it we UNDERSTATE it. Not interpolated, because **R o/m is NOT MONOTONIC: 1.0740 → 1.0613 → 1.0910 → 0.9225 → 0.9721**, and both component divergences CHANGE SIGN across the range. That non-monotonicity is the empirical case for D19 — a size-dependent divisor would have to be fitted to a curve that wanders. **HOW THE TWO DIVERGENCES COMBINE, and the answer is not "they add": R o/m = (D o/m) ÷ (V o/m).** Verified to 3dp on every file. **So the errors DIVIDE, and the worst cases are where they have OPPOSITE signs.** At 10 conn the denominator runs high (1.0366) while the view runs low (0.9501) and both push the ratio UP → 1.0910, the envelope's ceiling. At 355 conn the denominator runs low (0.9885) while the view runs high (1.0715) and both push it DOWN → 0.9225, the floor. **Four of the five files are in that compounding regime; only the zero-connection file has both errors in the same direction, where they partially cancel.** The combined bound is therefore **narrower than the naive product of the two extremes** (0.958–1.168) because the sign changes are correlated with size rather than independent — **but it is wider than either divergence alone, and it points toward the ENDS of the connectivity range, not the middle.** ck4's label has to be true at both ends. **CONSEQUENCE FOR ck4, stated plainly: "at least Nx" is FALSE at the floor and "exactly Nx" is false everywhere.** The honest framing is a measured estimate with a two-sided ±9% envelope, and the floor file must not carry hub-level confidence | **CLOSED 2026-09-04. Envelope 0.9225–1.0910 measured, two-sided, straddling 1.0; crossover bracketed at 10..355 conn. OPEN: no measurement inside that bracket; +0.009 headroom to the stop ceiling** |
| — | **PHASE 8 CHECKPOINT 3b-2 — THREE §17.16 ENTRIES ON THE 3a-quater MEASUREMENT, and D20.** **CORRECTION 1 — THE FIVE-FOR-FIVE DENOMINATOR MATCH IS AN ALGEBRAIC IDENTITY, NOT A VALIDATION, AND MUST NEVER BE CITED AS ACCURACY.** `D o/m` equalled `derived_divisor / 4.7` to 4dp on all five files (0.9598 · 1.0440 · 1.0366 · 0.9885 · 0.9996). **That is guaranteed by construction:** the derived divisors were themselves computed as `total_bytes / naive_tokens` from §4.2, and `D o/m = (total_bytes / 4.7) / naive_tokens`, so the two are the same expression rearranged. **It holds for any file, including one whose naive token count nobody has measured, and therefore predicts NOTHING about an unmeasured file.** Verified algebraically and numerically before being recorded, precisely because "matched on 5 of 5" is exactly the sentence someone will later quote as evidence. **Also the source of my three missed bands: they were DIVISOR-only predictions applied to the RATIO**, which additionally contains the view-side term. **CORRECTION 2 — THE CONSEQUENCE, and it reverses where the work should have gone.** Because the denominator's deviation is fully accounted for by that identity, **it carries ZERO unexplained error. Every bit of residual error in `R o/m` is VIEW-SIDE — `_estimate_tokens` against tiktoken (0.8936 to 1.0715, a 17.8-point spread).** **Four checkpoints (1c, 1d, 3a-ter, 3a-quater) hardened the denominator; the numerator is where the error actually lives and it has never been the subject of one.** **CORRECTION 3 — THE 1.10 STOP CEILING WAS ARBITRARY AND IS NOT A VALIDATED BOUND.** I reported "+0.0090 headroom" as though it were margin against something derived. It was a number I chose. **The honest claim is the measured envelope itself: 0.92–1.09, on FIVE PYTHON FILES, in ONE repository, at ONE SHA.** No population claim beyond that, and "headroom" should not appear in the record again unless something derives the bound. **D20 — THE VIEW-SIDE DIVERGENCE IS NOT TO BE FIXED. BINDING.** `_estimate_tokens` uses `_CHARS_PER_TOKEN` (3.6), which is what `_apply_budget` prices the MCP payload with — **so retuning it re-prices the 219.7x Phase 6 figure (D3) and moves the number the whole phase exists to report.** The divergence is bounded and labelled, exactly as the denominator's is. **Note the asymmetry this creates and accept it deliberately: the error now known to dominate is the one we have decided not to touch** | **RECORDED 2026-09-04. Identity flagged as non-evidence; residual error localised to the view side; the envelope's bound is measurement, not a validated limit** |
| — | **PHASE 8 CHECKPOINT 3b-2 — D21 (collapse), D22 (test tray), and T1's classification measurement. THE BOUNDARY IS CLEAN, and the one apparent leak is a FALSE POSITIVE that argues against the obvious classifier.** **D21 — SUBSYSTEM AGGREGATION REJECTED ON DISTRIBUTION, NOT COUNT.** 6 distinct values on 2256 (12 on 2419), comfortably under the ~15 threshold — **but one subsystem holds 258 of 274 (94.2%), which is exactly the importer count.** Collapsed it is a single bubble saying "258 files"; expanded it is 258 nodes. **Subsystem stays COLOUR only (D15), which 6 values serve well.** Importers collapse at **TWO-SEGMENT folder granularity**: 30 groups, largest 76, 12 shown individually with `fileCount`, remainder in one explicit aggregate. **Both alternatives were measured and rejected: top-level gives 2 groups (`superset` 134 / `tests` 124 — no reduction in information, total loss of it), full `dirnameOfPath` gives 95 groups with 50 singletons (more objects than the 12-largest cut, and half of them singletons).** Imports side: no collapse, 22 nodes. Reuses `groupByDirectory` / `CyNodeData.fileCount` / `dominantCluster` unchanged — nothing needed widening, and `dominantCluster` already excludes nulls from the vote rather than treating unclustered as a cluster. **D22 — `tests/` GETS ITS OWN TRAY.** 124 of 258 is the finding, not noise, and folding it in buries it; the split shrinks the source spread so the 12-largest cut covers more of what remains; and an engineer changing this file wants source importers first — 258 undifferentiated overstates the RELEVANT blast radius while staying arithmetically correct. **BINDING: 134 + 124 = 258 must be visible ON SCREEN, not inferable. Two trays that each look complete and sum to less than 258 is §17.25 moved into layout.** **T1 — CLASSIFY BY THE `tests/` PATH PREFIX, AND BY NOTHING ELSE. Measured on 2256's 258 importers: 124 `tests/` + 134 source = 258, exact.** Leakage under every basename pattern tried (`test_*.py`, `*_test.py`, `conftest.py`, path segment `tests`/`test`, `fixture`) is **exactly ONE file — and it is a FALSE POSITIVE**: `superset/commands/database/test_connection.py` is production code, a command that TESTS A DATABASE CONNECTION, not a test module. **The inverse check is the decisive one: 46 of the 124 files inside `tests/` match NO test-name pattern** (`access_tests.py`, `base_tests.py`, `api_tests.py`, `dashboard_utils.py`, `superset_factory_util.py` — Superset uses a plural `*_tests.py` suffix plus helper modules). **So a basename classifier would misclassify 1 production file as a test AND miss 46 real ones; the path prefix misclassifies nothing.** Recorded because the basename heuristic is the one anyone would reach for first, and it is wrong in both directions on this repo | **RECORDED 2026-09-04. Classify on the `tests/` prefix only — NEVER on basename. Boundary clean, 124 + 134 = 258 exact** |
| — | **PHASE 8 CHECKPOINT 3b-2 — BUILT. `NeighborhoodView.tsx` + `lib/contextLayout.ts`. Frontend 22 -> 24 files / 298 -> 322 tests, plus 4 Playwright specs; backend UNCHANGED at 1,512 passed / 1 skipped / 0 failed. SEEN IN A BROWSER, and two real defects were found there that no test caught.** All structure lives in `lib/contextLayout.ts` (pure, 19 tests) because vitest is `environment:node` and logic left in a `.tsx` is logic nothing verifies. Reuses `runElkLayout` / `computeElkLayout` / cytoscape unchanged; `DependencyGraph.tsx` untouched and a tripwire asserts it is not even referenced. **RECONCILIATION HOLDS AND IS ON SCREEN:** `imports 22 + imported by 258 (134 source + 124 tests) − both 6 = 274 connected files`, rendered as text, with tray chips reporting `16 / all 16 shown`, `134 / 12 shown + 14 folded (18)`, `124 / all 2 shown`. Breaks canaried: zeroing a group's `fileCount`, dropping an aggregate, and rendering the tests tray without the source tray all fail their assertions. **TWO DEFECTS THE BROWSER FOUND THAT THE SUITES DID NOT — the argument for D1 being a binding constraint rather than a preference.** (1) **The graph was illegible.** `runElkLayout` does call `cy.fit`, so nothing was broken; the star is ~20 nodes stacked vertically (~800px) and the container was 560px, so `fit` scaled it down until labels were unreadable. All 4 Playwright assertions and all 19 unit tests passed on that render. Fixed by sizing the container to its content (900px) and widening layer spacing. **A visual constraint cannot be discharged by a test that asserts a canvas exists.** (2) **TOKEN NUMBERS WERE ON SCREEN, violating this checkpoint's own instruction.** ck2's raw definition list was still rendering `view_tokens`, `connected_files_tokens`, `saved_tokens` and `saved_ratio` (270.4075464243382, unformatted). **REMOVED, not hidden** — it was a scaffold to prove the data arrived and the graph now does that job; ck4 reintroduces token figures deliberately, with a label the 0.92–1.09 envelope can support. **D22 IS ONLY PARTLY SATISFIED, and this is recorded rather than claimed.** The 134/124 split IS visible and not inferable — in the header line and the tray chips — which is D22's literal requirement. **But ELK stacks every importer in ONE column, so in the graph itself the two test groups sit at the bottom of the same column as the 12 source folders.** A `cut-rectangle` shape was added for `tray="tests"` (shape, not colour — colour is spent on subsystem per D15) and it is **visibly subtle at render scale**. Honest state: the split reads from the text, weakly from the graph. Separate visual lanes would need ELK layer constraints — deferred, not done. **MEASUREMENT CORRECTION to my own M3 (§17.16): I reported 30 two-segment folder groups across 2256's 258 importers. The real figure is 28 (26 source + 2 tests).** M3's ad-hoc `path.split("/")[:2]` counted the FILENAME as a segment for two-part paths, so `superset/x.py` became its own group. `folderKey` caps at `parts.length - 1` and is correct. D21's decision is unaffected — 26 groups with a 12-shown cut still beats 2 and 95 — but the number quoted in the D21 row is wrong by two. **The tests tray genuinely does not aggregate** (124 files in exactly 2 second-level folders: `unit_tests` 76 + `integration_tests` 48), so its total is directly countable on screen while the source tray's 134 needs its aggregate. Pinned as a test after my first break attempt targeted that tray and **could not fail** | **CLOSED 2026-09-04. Graph only, no token numbers. OPEN: D22's split is textual not spatial; null-subsystem colour still unverified on data (repo 6 is 100% covered)** |
| — | **PHASE 8 CHECKPOINT 3b-3 — §17.37 PROMOTED, D22 recorded as PARTIALLY SATISFIED, and the M3 count corrected.** **§17.37 — "A perceptual property cannot be discharged by a test of structural presence"** (`code-health-contract.md`). Promoted on the ck3b-2 evidence: **4 Playwright assertions and 19 unit tests passed against a render whose labels were unreadable** — an ~800px star fitted into a 560px container by a `cy.fit` call behaving exactly as written. **Nothing was broken; the instrument could not perceive the property it existed to check**, which is §17.30's shape one level up. Each assertion was true (`toBeVisible`, canvas count, container height, and the data itself) and their conjunction was accepted as evidence for a claim none of them addressed. **The rule: perceptual properties require OBSERVATION, and the observation is the evidence — recorded with what was seen, at what scale, not as "looked fine".** The corollary is in the contract too: a pixel-diff test is not discharge either, because it detects change rather than legibility and will lock in an unreadable baseline. **THIS BINDS ck4's BADGES.** **D22 — PARTIALLY SATISFIED, SPATIALLY DEFERRED.** The 134/124 split IS visible in the header line and the tray chips, which is the literal requirement and is met. **The graph carries it weakly**: ELK stacks every importer in one column, and the `cut-rectangle` shape on the tests tray is subtle at render scale. **ELK layer constraints are deliberately NOT attempted** — that is real risk to a shared, unchanged layout path (`elkLayoutRun` / `computeElkLayout`, used by Dependency Graph as well) in exchange for a split the text already carries. Recorded as a known partial rather than closed. **M3 CORRECTION (§17.16) — THE TWO-SEGMENT GROUP COUNT IS 28 (26 source + 2 tests), NOT 30.** My ad-hoc `path.split("/")[:2]` counted the FILENAME as a segment for two-part paths, so `superset/x.py` became its own group. **D21 was therefore decided on a figure wrong by two; the conclusion is unaffected** — 26 groups with a 12-shown cut still beats 2 (all information lost) and 95 (50 singletons). **`folderKey` in `lib/contextLayout.ts` is now the SINGLE source for group counts, so nobody derives them a third way** — which is how two of the three figures in this decision came to disagree in the first place | **RECORDED 2026-09-04. §17.37 in the contract; D22 partial and spatially deferred; group counts have one owner** |
| — | **PHASE 8 CHECKPOINT 3b-3 — BUILT. Navigation, the D14 fingerprint, D23's empty state, and /ranking's `limit`. Frontend 24 -> 26 files / 322 -> 336 tests + 9 Playwright specs; backend 1,512 -> 1,518 passed / 1 skipped / 0 failed. OBSERVED IN A BROWSER per §17.37, and the observation found one defect.** **D14 — THE FINGERPRINT WORKS AND WAS SEEN FAILING.** `?fp=` is a 6-char FNV-1a base36 hash of the path, compared against the path the endpoint returns. **FNV rather than `crypto.subtle.digest` because the latter is async, which would push the comparison into an effect and let the mismatch state race the render.** **Zero collisions across all 518 distinct paths in both fixtures**, and the tradeoff is stated in-file rather than implied (~2^30 space, ~1-in-50,000 birthday risk at 6,584 files — not zero, and not the right tool if it ever needs to be). **Absent `fp` is `unverified`, NOT a mismatch** — refusing to render would break every link made before this checkpoint. Observed: `?fp=zzzzzz` on 2256 renders "This link points at a file that has changed", naming the link fingerprint (`zzzzzz`), the path the id now resolves to, and its actual fingerprint (`nv1o08`), with the graph NOT rendered and an explicit "show it anyway" escape. **CLICK DECISION, RECORDED: a single-file node NAVIGATES; a collapsed group DRILLS IN (its files listed below the graph as links); the centre says "this is the file you are looking at".** No branch is a silent no-op. **Expanding a group inside the graph was rejected: `unit_tests` holds 76 files, and re-laying-out 76 nodes in a container sized for ~20 is the §17.37 legibility defect again.** Observed working: clicking `commands (35)` listed all 35 paths, navigable. **And the list visibly contains `superset/commands/database/test_connection.py` on the SOURCE side — T1's false positive, correctly classified by the path prefix, confirmed in the UI rather than only in a test.** **D23 — THE EMPTY STATE OFFERS BOTH ENDS, and the numbers justify it starkly.** Rank ordering puts `superset/__init__.py` at rank 1 and `utils/core.py` at rank 3 — the two most flattering files in the benchmark. **`scripts/__init__.py` is rank 3,736 of 6,584 and could never appear in any top-N list.** A starting point that can only reach hubs misrepresents the feature by construction, through a mechanism that looks neutral. The floor is therefore offered explicitly and labelled "the other end of the range", with its rank stated. Pinned as a backend test asserting it is absent from the top 10 and ranks >100. **A DEFECT THE BROWSER FOUND — REPORTED, NOT IMPROVISED, per the stop condition.** `scripts/__init__.py` RENDERS correctly at every level that was specified: the no-connections notice, all trays reading 0, the reconciliation reading all zeros, unresolved (0). **But the 900px graph canvas is empty except for the single centre node, clipped at the top-left corner** — `cy.fit` on one node with no edges has nothing to fit against. **A zero-connection neighbourhood needs a distinct treatment and that treatment is NOT invented here**; the minimal option is to not render the canvas at all when `connected === 0`, since the notice already carries the whole answer. **Left for a decision, because the stop condition reserved it.** **BACKEND — `/ranking?limit=N` (D13), additive.** `None` means uncapped, so existing callers are unchanged (canaried: omitting it returns all 6,584 with `limit: null`, `truncated: false`). Applied AFTER ordering, never before — a top-N that sliced first would return N arbitrary files under a name promising the N that matter. Returns `total_before_limit` / `truncated` so a 10-of-6,584 response cannot be mistaken for a 10-file repo. **A FLAKY PLAYWRIGHT TEST WAS DELETED RATHER THAN KEPT GREEN.** It clicked a guessed pixel coordinate to hit a collapsed group, so its verdict depended on where ELK happened to place the node — §17.37's warning applied to an interaction instead of a rendering. The decision is covered deterministically in `contextNav.test.ts`; the drill-in itself is covered by observation, which is what §17.37 asks for. **Exposing the cytoscape instance on `window` to make it scriptable was rejected: production surface added for a test.** **One of my own assertions was wrong and the test caught it:** I asserted >600 distinct paths across the two fixtures by adding 275 + 356 without deduping. The real figure is **518** — 2419 is itself one of 2256's neighbours | **CLOSED 2026-09-04. Navigation, fingerprint, empty state, limit. NO token numbers on screen. OPEN: the zero-connection canvas needs a treatment decision; D22 still spatially deferred; null-subsystem colour still unverified on data** |
| — | **PHASE 8 CHECKPOINT 4 — D24–D27, and Z1: THE ZERO-CONNECTION STATE IS 16.2% OF THE REPO, NOT AN EDGE CASE.** **Z1 — MEASURED ON repo 6 @ `a05a0999`: 1,068 of 6,584 files have ZERO connected files. 16.2%. Roughly one file in six.** Spread across every language the atlas parses — **551 Python, 281 TypeScript, 192 TSX, 44 JavaScript** — so it is NOT a Python-only artifact and NOT one extractor misbehaving. **THE READ: the empty-neighbourhood state is a PRIMARY state and has been treated as polish.** Land on a random file and there is a ~1-in-6 chance the answer is "nothing connects to this". **AND THIS IS D23's SELECTION BIAS ONE LEVEL UP, which is the part worth keeping.** A rank-ordered empty state can never surface ANY of these 1,068 — the best-ranked one is `scripts/cypress_run.py`, and the rest are `scripts/translations/*`, `RELEASING/verify_release.py`, `superset-frontend/src/views/index.tsx`. **The mechanism that hid the honest floor from the starting list is the same mechanism that hid the SIZE of the zero-connection population from us: rank ordering is not neutral, and every view built on it inherits the same blind spot.** D23 fixed the symptom on one file; Z1 shows the population behind it. **D24 — A ZERO-CONNECTION FILE RENDERS NO CANVAS.** `cy.fit` on one node with no edges has nothing to fit against, and the observed result was a 900px empty box with a clipped corner node — **reading as broken software on the one file that carries the demo's honesty claim.** The notice holds the whole answer. **It must look deliberate, not degraded**, and at 16.2% it will be seen often. **D25 — DISPLAY PRECISION, AND ROUNDING NEVER FAVOURS US.** `270.6303` never reaches the screen: seven digits on a number with a measured ±9% envelope is false precision on the feature's most attackable artifact. **270.6303 → `~270x`; the floor 0.9940 → `~0.99x`, NEVER `~1x`.** Any savings claim rounds toward the conservative direction. **D26 — THE BACKEND SUPPLIES THE DISPLAY STRINGS** (`saved_ratio_display`, `envelope_pct`). Two reasons: it keeps D7's no-frontend-arithmetic tripwire at **zero exceptions** — no narrowing, no false-positive risk, which is the lesson from the `or 0` tripwire that flagged correct code on its first catch — and it keeps ONE producer for the number. Same Option B pattern as `connected_index`. **D27 — PER-FILE FIGURES ONLY. NEVER POOLED OR REPO-WIDE.** The deck's 219.7x is **pooled tiktoken across five files**; the screen's ~270x is **one file by the char estimator**. Different quantities, different instruments — putting them where they appear to compete invites a comparison that is meaningless and that we would lose. **`estimator_vs_measured` stays OUT of the UI entirely**: it exists only where a benchmark exists, so it means something different per file, which is §17.25 by construction | **RECORDED 2026-09-04. Z1 reframes the empty state as primary (16.2%). Rounding is one-directional by decision** |
| — | **PHASE 8 CHECKPOINT 4 — BUILT. Token badges, and PHASE 8 IS FEATURE-COMPLETE. Backend 1,518 -> 1,527 passed / 1 skipped / 0 failed; frontend 336 -> 347 tests / 26 files; Playwright 9/9. OBSERVED IN A BROWSER per §17.37.** **D26 SURVIVED A COLLISION WITH D7, AND THE FIELD NAME GAVE WAY RATHER THAN THE TRIPWIRE.** D26 specified `saved_ratio_display`; D7's tripwire bans the substring `saved_ratio` in frontend source, so a component printing that field trips it — **a FALSE POSITIVE, since printing a string is display, not arithmetic.** The choice was narrow the tripwire or rename the field. **D26's own stated reason for putting these strings in the backend was to keep the tripwire at ZERO exceptions, citing the `or 0` incident — so the reason outranked the name.** Renamed to **`ratio_display`**, and the component counts to **`graph_cost_display` / `read_cost_display`** for the same constraint. **D7's tripwire passes literally unchanged, still zero exceptions.** Recorded as a deviation from D26's letter in service of D26's purpose. **D25 — ROUNDING IS ONE-DIRECTIONAL, and the canary proves the naive version is dishonest.** `math.floor` on the scaled value, never `round`: **`round(0.9940, 1)` is 1.0**, which turns "the graph costs MORE here" into "break-even" and erases the one result Phase 6 reports in order to be honest. Canaried in both directions — the test asserts the naive form produces `~1.0x` and `~271x` while ours produces `~0.99x` and `~270x`. Every band cuts downward. **A CONSEQUENCE WORTH RECORDING: 2256 (270.4075) and 2419 (270.6303) BOTH display `~270x`.** That is correct, not a collision — the 0.22 between them is far inside the measured −8%/+9% envelope, so showing it would be the false precision D25 exists to prevent. Pinned as a test so nobody "fixes" it into more digits. **D24 — NO CANVAS AT ZERO CONNECTIONS, and the legend went with it.** The graph legend was still rendering over a file with no graph; gated too. Observed: `scripts/__init__.py` now ends cleanly at the notice. **D27 held** — a new `?raw` tripwire across five source files bans the pooled figures (219.7, 3,436,264, 15,644, 99.5%) and `estimator_vs_measured`, canaried both ways. **THE OBSERVATION, which is the evidence (§17.37).** 2256: `~270x` at 2xl weight, "cheaper than reading every connected file", envelope "estimate, −8% / +9% against tiktoken on the five measured files", and beneath it **"this view costs 5,062 tokens · reading every connected file would cost 1,368,803 tokens"** — so the ratio is checkable rather than asserted. **The floor file is the one that matters and it reads right: `~0.99x` in amber, "no saving on this file — the graph costs slightly more than reading it", and "this view costs 168 tokens · reading every connected file would cost 167 tokens" — 168 against 167, the loss legible in the components, not just in the ratio.** It reads as a deliberate honest answer, not a bug and not a blank. Per-view line confirmed updating across navigation (2419 → 6,380 / 1,726,621; 2256 → 5,062 / 1,368,803). **TWO OF MY OWN ERRORS, both caught by tests rather than by me.** (1) I asserted 2419's display as `~268x` — that was its PRE-D18 ratio, and I had the two files' raw values swapped. (2) My badge screenshot caught a "Loading…" frame twice because it slept on a fixed timeout instead of waiting for the badge — **the same sampling mistake §17.30's fourth instance records, made while implementing the checkpoint that §17.37 binds.** Fixed by waiting on the element | **CLOSED 2026-09-04. PHASE 8 FEATURE-COMPLETE: 14/14 checkpoints. OPEN: D22's spatial split; null-subsystem colour unverified on data; Phase 8.5 source viewer deferred** |
| — | **PHASE 8 CHECKPOINT 4-bis — D28: THE "0.93x–293x HONEST SPREAD" HAS AN UNEXAMINED DENOMINATOR, and it is §17.5c applied to our own headline claim.** **THE CLAIM AS STATED IS A CLAIM ABOUT THE REPOSITORY. THE EVIDENCE IS A CLAIM ABOUT FIVE FILES.** The spread derives entirely from §4.2's five benchmark files, which were selected by CONNECTIVITY from the graph — a leaf, two mid-connected, two hubs. **Z1 established that 1,068 files (16.2%) have zero connections, and no connectivity- or rank-ordered selection can reach any of them.** So **0.93x is the floor AMONG SAMPLED FILES, not the floor among the repository's files**, and the difference is exactly the one §17.5c exists to name: a statistic whose population differs from the one it is quoted about. **THE IRONY IS WORTH RECORDING BECAUSE IT IS INSTRUCTIVE.** "Read the distribution, not the pooled number" has been Phase 6's own discipline since checkpoint 3, and it was applied rigorously to the five files while the question of which five was never reopened. **A distribution reported honestly over a population chosen unexaminedly is still a §17.5c error — the rigour was spent one level below where the problem was.** **WHAT IS AND IS NOT AFFECTED.** The PER-FILE UI is correct and unaffected: it computes and displays one file's own figure, and ck4 verified it reads honestly at the floor (`~0.99x`, "no saving on this file"). **It is the SUMMARY claim — the one on a slide — that is unsupported as phrased.** No per-file number needs changing; a sentence does. | **RECORDED 2026-09-04. Per-file display unaffected; the summary claim needs re-derivation over the whole population, which 4-bis measures** |
| — | **PHASE 8 CHECKPOINT 4-bis — MEASURED, and it found a REAL DEFECT IN THE PER-FILE UI, not just in the summary claim. D28 said the per-file display was unaffected; that is now RETRACTED (§17.16). Suites unchanged: backend 1,527 / 1 skipped / 0 failed, frontend 26 files / 347 tests. No source changed.** **THE DEFECT — A ZERO-CONNECTION FILE CAN DISPLAY A LARGE FICTITIOUS SAVING.** `superset-frontend/src/pages/AlertReportList/AlertReportList.test.tsx` has **0 connected files** and the UI shows **`~10.9x`, captioned "cheaper than reading every connected file" — a sentence about a set that is empty.** **MECHANISM:** D18 puts the centre file's own bytes in the denominator, which is correct when there ARE connected files (it matches §4.2's naive cost) — but with zero connections the denominator collapses to the file ITSELF, so the ratio becomes `(own_bytes / 4.7) ÷ (cost of a neighbourhood that says "no neighbours")`. **The numerator scales with FILE SIZE while the denominator is near-constant (~170–460 tokens of mostly-empty JSON), so a big unconnected file shows a big "saving": 18,067 bytes → 3,844 read-tokens against 351 graph-tokens = 10.95x.** **IT IS FICTITIOUS: the neighbourhood contains nothing about the file's CONTENTS, so it cannot substitute for reading it.** The 219.7x claim rests on substitution — read the neighbourhood INSTEAD OF the connected files — and with no connected files there is nothing to substitute. The comparison degenerates into two different questions. **ck4's floor display was right BY ACCIDENT: it happens to hold for SMALL unconnected files (ratio < 1) and inverts for large ones.** **1. ZERO-CONNECTION SAMPLE, n=41 of 1,068, proportional across all four languages, 0 endpoint failures.** **NEW FLOOR: 0.9278x** on `tests/unit_tests/mcp_service/semantic_layer/tool/__init__.py` — **below the published 0.93x**, so the headline's floor was itself set by the sampled five. Median 1.5327; **30 of 41 exceed 1.0x and 15 exceed 2.0x, up to 10.95x** — all of them by the mechanism above. The sub-1.0x cases are uniformly tiny `__init__.py` files (785 bytes → 167 read-tokens against ~172–180 graph-tokens). **3. WHOLE-REPO DISTRIBUTION — random sample n=600 of 6,584 (9.1%), seed 41, 4.9 min, 0 failures.** All 6,584 would take **~52 min at 0.49 s/call**; sampled rather than changing code to make it feasible, per the stop condition. **ALL FILES: p10 1.66 · p50 10.34 · p90 94.69** (p25 4.58, p75 28.07, p99 204.31), min 0.9278, max 277.28, **6.0% below 1.0x, 9.5% above 100x**. **WITH CONNECTIONS (83.2%): p10 3.65 · p50 13.74 · p90 106.18, and 0.0% below 1.0x.** **ZERO CONNECTIONS (16.8%): p10 0.95 · p50 1.40 · p90 7.46, 35.6% below 1.0x** — and the 64% above 1.0x in that stratum are the fictitious ones. Sample's zero-connection share 16.8% against Z1's population 16.2%, so the draw is representative. **4. WHAT THE DEMO CAN NOW CLAIM.** **Supported:** on files that HAVE connections — 83.8% of the repo — the median saving is **~14x**, the 10th percentile is **~3.7x**, and **not one sampled connected file fell below 1.0x**. That is a stronger and better-founded claim than the five-file spread, because its population is stated. **NOT supported:** the "0.93x–293x" range as a statement about the repository — both endpoints came from the sampled five, and both are wrong (the real floor is 0.9278x, the sampled max here is 277x). Also NOT supported: any saving figure at all for the 16.2% with no connections, where the ratio does not measure substitution. **The honest framing is per-stratum: a real median saving where the graph has something to say, and no claim where it does not** | **CLOSED 2026-09-04. Measurement only. OPEN and now URGENT: the per-file UI overstates on large zero-connection files — D28's "per-file unaffected" is retracted** |
| — | **PHASE 8 CHECKPOINT 5 — §17.16: A RECURRING PATTERN IN MY PRE-REGISTRATIONS, recorded as ONE pattern rather than two incidents. Plus D29–D31.** **THE PATTERN: I SAMPLE ONE END OF A RANGE AND TREAT IT AS THE RANGE.** Two instances, and naming them together is the point — separately each reads as a one-off, together they say how my predictions should be read. **(1) ck1d — I discarded the 4.511 divisor datapoint as "tiny-file noise".** It was a real measurement, and it was the datapoint governing the floor file; the four survivors ran high on small connected sets and low on large ones, so I removed the small end of a **size trend** as an outlier. **(2) D28 — I claimed the per-file UI displayed the zero-connection case correctly.** I validated **one** file (`scripts/__init__.py`, 785 bytes, 0.994x) and generalised to the population. **The ratio INVERTS with file size**: at 18KB the same code path shows `~10.9x`. Retracted. **Both are the same error at different scales — a conclusion drawn from one end of a distribution I had already been told was a distribution.** "Read the distribution, not the pooled number" has been this project's discipline since checkpoint 3, and I have now broken it twice in my own pre-registrations while enforcing it on the code. **Future pre-registrations from me should be read as untested at the ends unless the ends were measured**, and the earlier missed bands (three of three at 3a-quater, all in one direction) are consistent with the same habit. **D29 — NO RATIO WHEN `connected_files_distinct === 0`. Not clamped, not floored, not amber. ABSENT.** The ratio is arithmetically correct and semantically void: the 219.7x claim rests on **substitution**, and with nothing to substitute the numerator and denominator answer different questions. **Clamping to 1.0x would replace a meaningless number with a PLAUSIBLE one, which is §17.25 exactly** — the failure mode is the consumer being unable to tell. Component costs STAY (3,844 read vs 351 graph): they are real, checkable, and they are what a reader actually needs. **This RETIRES ck4's amber floor display**, which was right by accident for small files and inverted for large ones — `scripts/__init__.py` stops showing `~0.99x` too. **D30 — DEMO CLAIMS ARE PER-STRATUM, WITH THE STRATUM STATED.** "On the 83.8% of files that have connections: median ~14x, p10 ~3.7x, no sampled file below 1.0x. On the 16.2% with no connections the ratio does not measure substitution and no saving is claimed." **D31 — "0.93x–293x" IS RETIRED AS A REPO-LEVEL CLAIM.** Both endpoints came from §4.2's five files and both are wrong (real floor 0.9278x; this sample's max 277x). It survives ONLY as the spread across those five benchmarked files, never as a statement about the repository | **RECORDED 2026-09-04. The one-end-of-a-range habit is named across both instances; ratio suppressed at zero connections** |
| — | **PHASE 8 CHECKPOINT 5 — BUILT. The meaningless ratio is suppressed, and LOOKING AT THE PAGE FOUND A SECOND INSTANCE OF THE SAME DEFECT ONE LAYER DOWN. Backend 1,527 -> 1,534 passed / 1 skipped / 0 failed; frontend 26 -> 27 files, 347 -> 357 tests; Playwright 9/9.** **D29 IMPLEMENTED AS `null`, NOT A SENTINEL.** `ratio_display` is `None` when `connected_files_distinct == 0`, so **there is no string the frontend could render as a number even by accident** — a sentinel like "n/a" would still be a value someone could style like a figure. The reason travels with it in `ratio_absent_reason`, supplied by the backend (D26), so the browser prints rather than composes. **The raw `saved_ratio` is still computed and still in the payload** — it is the DISPLAY that is suppressed, so the number stays auditable. **CANARIES, both observed failing.** Restoring the unconditional call brings the defect straight back — **`~10.9x` on `AlertReportList.test.tsx` and `~0.99x` on `scripts/__init__.py`**. Suppressing universally strips 2256's `~270x`, so the **regression guard proves the suppression is SCOPED to zero connections rather than applied everywhere**. **THE SECOND DEFECT, FOUND BY LOOKING AND NOT BY ANY TEST (§17.37).** With the ratio gone, the costs sentence still read **"reading every connected file would cost 3,844 tokens" — about a set with nothing in it, where 3,844 is the cost of reading THIS file.** The ratio fix suppressed the NUMBER and left the SENTENCE around it. **Same false-caption class, one layer down, and it survived the fix aimed at its sibling.** Closed by moving the whole sentence to the backend as `costs_line`: "reading this file would cost" at zero connections, "reading every connected file would cost" otherwise. **The frontend no longer composes any claim about the numbers — it prints one string.** **THE OBSERVATION (the evidence).** `AlertReportList.test.tsx` now reads: **"No saving figure for this file. This file has no connected files, so there is nothing for the graph to substitute for. A ratio here would compare the cost of reading the file against the cost of being told it has no neighbours — two different questions. The costs below are still real."** followed by "this view costs 351 tokens · reading this file would cost 3,844 tokens", the three zero-trays, the no-connections notice, and 14 unresolved specifiers. **No canvas, no ratio, no amber near-1x framing.** It reads as a deliberate factual answer, not a missing number and not a failed load. `scripts/__init__.py` the same, with 168 / 167. 2256 unchanged at `~270x`. DOM-verified across all three: ratio-node count 0 / 0 / 1. **Two ck4 tests were SUPERSEDED and updated with §17.16 markers rather than deleted** — one asserted the floor file shows `~0.99x`, the other that every file carries a ratio string. In both the property under test survives and now holds more strongly: the file makes no claim it cannot support. Conservative rounding is still covered directly against the formatter | **CLOSED 2026-09-04. PHASE 8 COMPLETE — 16/16 checkpoints** (15 with rows above, plus ck0 reconnaissance, which predates this record and has none; "15" here first undercounted it). OPEN: D22's spatial split; null-subsystem colour unverified on data; Phase 8.5 source viewer deferred** |
| — | **PHASE 8 — CONTEXT VIEW LAYOUT PASS (not a checkpoint; a usability fix from real use). Backend unchanged at 1,534 / 1 skipped / 0 failed; frontend 27 files / 357 tests; Playwright 9/9.** **FOUR PROBLEMS, ALL FOUND BY USING THE PAGE RATHER THAN TESTING IT — §17.37's point restated by the user.** **(1) THE SCROLL TRAP, the worst of them.** cytoscape captures the wheel by default, so the page stopped scrolling the moment the cursor crossed the graph and the only way past was the scrollbar. **Fixed with `userZoomingEnabled: false`: the wheel now belongs to the PAGE, and zoom moved to explicit +/−/fit controls.** Drag-to-pan is untouched, which is the interaction people actually reach for inside a graph. Verified in a browser: `scrollY` 0 → 226 with the cursor over the canvas. **No test caught this because no test scrolls.** **(2) THE GRAPH ATE THE PAGE.** A 900px full-width canvas pushed the trays, the drill list and the unresolved tray below the fold. **Now two columns: the graph holds a tall panel on the left, and every piece of TEXT lives in an independently scrollable rail on the right.** Unresolved is reachable without scrolling the page at all. **(3) IT DID NOT FIT THE SCREEN.** A fixed `calc(100vh - 190px)` has to GUESS the repo header and tab strip height, and it guessed low — the graph ran off the bottom, which is what the layout existed to stop. **Replaced by measuring the panel's own `offsetTop` and re-measuring on resize.** Verified: bottom edge at 875px in a 900px viewport. **(4) THE COUNTS WERE UNREADABLE** — 11px grey mono. Now three colour-coded cards with the NUMBER first at 20px: teal imports, indigo source importers, amber test importers. **The tray accents deliberately sit apart from the subsystem palette: those colour the NODES, these colour the DIRECTION, and conflating them would be two meanings for one channel.** **A WORDING BUG THE REDESIGN EXPOSED:** the sub-line read "10 … all 3 shown", mixing FILES with FOLDERS so it parsed as a contradiction. Now "in 3 folders, all drawn". One Playwright assertion updated to match; the property it tests is unchanged | **DONE 2026-09-04. Usability only — no payload, no numbers, no decisions changed** |
...
| **8 — GRAPH AS CONTEXT, THE USER SURFACE** | **DEFINED AND OPENED 2026-09-04 — the placeholder is spent by a human decision.** Checkpoint 0 (recon) done; checkpoint 1 (the `context` endpoint) unblocked, NOT built. Turns Phase 6's queryable graph into a surface a person can use: a **Context tab in `RepoDetail`** whose content must be self-legible, with the token-savings number prominent (binding constraint on checkpoints 3–4). | route `GET /{repo_id}/files/{file_id}/context` (option (a), precedent-matching, no URL-encoding); denominator computed by the endpoint itself (Option B — the MCP payload is NOT widened, so the 219.7x figure is not re-priced); `neighbors` vs `context` return different counts for the same file BY DESIGN. Decisions rows in section J |
| **9 — drift detection** | **UNNUMBERED-UNTIL-DESIGNED, ~10% designed.** Moved here 2026-09-04 to free Phase 8 for the UI work. **Holds the number provisionally and should not be treated as a planned phase** — the Phase 8 placeholder lesson applies to it now: ask what it means rather than infer a scope. | no files, no migrations, no tests, no written scope |
| — | *(superseded 2026-09-04, kept per §17.16)* **NOT STARTED AND NOT DEFINED** | no files, no migrations, no tests — **and no scope has ever been written for it.** The number is a placeholder in this table, not a planned phase. A fresh session should treat 'Phase 8' as unallocated and ask what it is rather than inferring a scope from the numbering |

## Suite state

- **Backend: 1,196 passed / 0 skipped / 0 failed** on the Linux VM, full run
  **2026-09-01 (81.2s)** — re-run during the checkpoint-5 diagnosis with **no code
  touched**. **Superseded figure, kept per §17.16: 1,181** (2026-08-26, 78.6s). The
  delta is **exactly the 15 tests in `test_read_hook.py`**, which were committed with
  the checkpoint-5 hook and never recorded here — so the stale total was itself
  evidence that the record trailed the code. `athena_read_hook.py --selftest` also
  passes (parsed extension table MATCHes the authoritative import). Earlier context
  for the 1,181 figure follows.
- **Backend: 1,196 recorded** — 1,181 plus checkpoint 5's 15 `test_read_hook.py` tests, reconciling exactly. **[NOT RE-RUN 2026-09-03: this figure is carried from the checkpoint 5 row, not re-measured, and is stamped per §17.16 rather than presented as a fresh run. The 1,181 reconciliation below is the last figure actually observed.]** Previously **1,181 passed / 0 skipped / 0 failed** on the Linux VM, full run
...
Filed alongside partial-input and the check-that-cannot-fail. Two instances
would be a coincidence; there are three, and they share a mitigation the other
two classes do not.

**The shape: an instruction that assumes infrastructure or configuration which
has never been verified to exist.** It produces no error, because the thing that
would raise the error is the very thing that never runs.

**Instance 1 — the `piper-phonemize` pin.** `piper-tts==1.2.0` required a
package with no Python 3.12 distribution, on a project pinning 3.12.8. It stayed
invisible because it lived in an optional extras file nobody installed: *an
unresolvable pin in a file that is never resolved produces no error.*

**Instance 2 — the confidence label on Gemini's rate limit.** "250 RPD, medium
confidence" was assigned from how many secondary sources agreed, which measured
source-copying rather than truth. The real figure was 20. No amount of reading
the sources would have caught it; they were downstream of each other.

**Instance 3 — Hard Constraint #2's Docker assumption.** "Bake model weights
into the Docker image at build time" was carried verbatim across four filed
items — the original voice-migration constraint, KI-2, VKI-4, and a Phase 5
go-ahead — while `docs/DEPLOYMENT.md` states the project *deliberately avoids
Docker dependence* and no Dockerfile exists. Each downstream item inherited the
framing from the one above it. Caught only when an implementer tried to execute
against it.

**The mitigation is upstream, and that is what makes this a separate class.**
Partial-input and check-that-cannot-fail are both fixed by discipline in how
code is written — operate on whole inputs; break a guard and watch it fail.
Neither helps here, and review does not either: all three instances survived
review precisely because the text was internally coherent. What catches an
unresolved premise is **verifying that the target of an instruction exists
before writing the instruction against it** — `pip index versions` before
pinning a version, response headers before quoting a rate limit, `ls Dockerfile`
before writing "bake into the image".

One thing all three classes share, worth stating once: **each is a self-report
that is internally consistent and externally false.** Code describing what it
does while doing something narrower; a test describing what it verifies while
verifying nothing; an instruction describing a system that is not there.

## Voice migration Phase 5 outcome

Closed KI-2 and VKI-4 under one change, on the deployment architecture that
actually exists.

**KI-2 needed a code change, not configuration.** FastEmbed honours no
environment variable for its cache location — not `FASTEMBED_CACHE_PATH`, not
`HF_HOME`, not `XDG_CACHE_HOME` — only the `cache_dir` constructor argument, so
both call sites (`vector_store.py`, `codebase/embeddings.py`) now pass it.
Measured along the way: its default was **`/tmp/fastembed_cache`**, which is
worse than the once-per-deploy download KI-2 describes, because `/tmp` is
cleared under a running service rather than merely between deploys.

**`local_files_only` turned the guarantee from incidental into structural.**
Both faster-whisper and FastEmbed accept it, and it was verified to genuinely
block an uncached fetch rather than being decorative. With `MODELS_OFFLINE`
defaulting on, a runtime fetch is now *impossible* rather than merely
unnecessary — the difference between "it did not download this time" and a
property a test can assert.

**A fourth instance of the partial-vs-whole staging defect happened in this very
commit** and is recorded rather than quietly fixed: `git add -A backend/app`
staged nine files of pre-existing working-tree WIP alongside four lines of mine,
in the commit that files the rule against exactly that. Caught on reading
